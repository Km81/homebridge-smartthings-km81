...
class SmartThingsClient {
  constructor(log, api, config) {
    ...
    axiosRetry(this.client, {
      retries: 3,
      retryDelay: (n, error) => {
        // v2.3.6 — 네트워크 오류면 response가 없어 늘 `undefined`가 찍혔다(무의미).
        // 상태코드가 없을 땐 원인(ENOTFOUND 등)을 대신 보여 준다. 클라우드 장애 때는
        // 기기 수만큼 반복되므로 debug로 내린다 — 진짜 실패는 호출부가 따로 경고한다.
        const why = error.response?.status || error.code || error.message || '원인 불명';
        this.log.debug(`SmartThings API 재시도 (${n}회차) — ${why}`);
    ...
    // v2.1.2 — off 재시도 취소 가드의 사각 봉합: 위 retryCondition은 "실패 시점"에 1회만
    // 평가되므로, 백오프 대기(1~7s) 중에 도착한 ON은 보지 못하고 이미 예약된 재시도가
    // 그대로 발사돼 사용자의 ON을 조용히 뒤집는다(감사 시나리오 I에서 실증). 재시도 요청이
    // 실제로 나가는 순간(요청 인터셉터)에 역전 여부를 한 번 더 검사해 뒤늦은 off를 차단한다.
    // 차단 시 off 프라미스는 거부되지만, 액세서리 계층 _scheduleOffRetry는 _offIntentTs
    // 의도 가드(ON이 이미 해제)로 무해하게 소멸한다.
    this.client.interceptors.request.use((cfg) => {
      const retryCount = cfg['axios-retry']?.retryCount || 0;
      if (retryCount > 0 && cfg._deviceId && cfg._sentAt && SmartThingsClient._isIdempotentOffCommand(cfg)) {
        const lastNonOff = this._lastNonOffCmdTs.get(cfg._deviceId) || 0;
        if (lastNonOff > cfg._sentAt) {
          this.log.warn(`[${this._labelOf(cfg._deviceId)}] switch:off 재시도 취소 — 백오프 중 같은 기기로 다른 명령이 전송되었습니다.`);
          return Promise.reject(new Error('off 재시도 취소(백오프 중 반대 방향 명령)'));
        }
      }
      return cfg;
    });
    ...
    this._deviceLabels = new Map();      // deviceId → 사람이 읽는 이름 (로그에 UUID 대신 표시, v2.1.0)
  }
  ...
  async init() {
    try {
      ...
      this.log.info(`OAuth 토큰 로드됨 (${TOKEN_FILENAME})`);
      return true;
    } catch {
    ...
    try {
      ...
      try {
        ...
        if (unlinkErr.code !== 'ENOENT') {
          this.log.warn(`임시 토큰 파일을 지우지 못했습니다 — 직접 삭제하세요: ${tmpPath} (${unlinkErr.message})`);
        }
      }
  ...
  async getStatus(deviceId) {
    ...
    const p = this.client
      ...
      .then((res) => {
        ...
        if (streak) {
          this.log.info(`[${this._labelOf(deviceId)}] 상태 조회 복구 — 연속 ${streak}회 실패 후 정상화`);
      ...
      .catch((e) => {
        ...
        // v2.3.6 — ①UUID 대신 기기 이름. 로컬 전송 기기는 **복구가 로컬 경로에서** 일어나
        // 라벨로 찍히는데 실패만 UUID로 찍혀, 감시 스크립트가 짝을 못 찾고 오탐했다(7/29 실사고).
        // ②message가 비면 "상태 조회 실패: "로 끝나 원인이 사라졌다 → 대체 문구를 넣는다.
        const why = e.message || e.code || String(e.response?.status || '').trim() || '원인 불명';
        const who = this._labelOf(deviceId);
        if (streak === 1) this.log.error(`[${who}] 상태 조회 실패: ${why}`);
        else if (streak % 10 === 0) this.log.warn(`[${who}] 상태 조회 실패 지속 x${streak}: ${why}`);
        else this.log.debug(`[${who}] 상태 조회 실패 x${streak}: ${why}`);
        // ★플래그를 새 Error로 옮겨 싣는다(v2.4.5 감사 C). 예전엔 여기서 통째로 버려서,
        //   재인증 창에서 조용히 넘기려던 `Laundry.js`의 `_noToken` 분기가 **도달 불가 코드**였다.
        //   그래서 토큰 재발급 몇 분 동안 기기마다 폴링 warn이 쏟아졌다.
        const wrapped = new Error(`[${who}] 상태 조회에 실패했습니다.`);
        if (e._noToken) wrapped._noToken = true;
        if (e._transient) wrapped._transient = true;
        throw wrapped;
      })
  ...
  // 로그용 기기 이름 등록 — 없으면 로그에 UUID가 그대로 노출된다 (v2.1.0)
  registerDeviceLabel(deviceId, label) {
    if (deviceId && label) this._deviceLabels.set(deviceId, label);
  }

  // ★v2.3.6 — 옵셔널 체이닝은 필수다. 이 함수는 **오류 처리 경로에서도** 불리므로
  // 여기서 던지면 원래 오류가 가려진다(실제로 이 버전에서 상태 조회 실패 로그에 쓰기
  // 시작하자마자 테스트 하네스가 죽었다 — 부분 스텁엔 _deviceLabels가 없었다).
  _labelOf(deviceId) {
    return this._deviceLabels?.get(deviceId) || deviceId;
  }

  // 명령 하나를 한국어 '항목 → 값'으로 (v2.1.1) — SmartThings 능력/명령 식별자를 사람 말로.
  // 이 플러그인이 실제로 보내는 5종을 매핑하고, 미지의 명령은 capability.command(args)로 폴백.
  static _fmtCommand(c) {
    const arg = Array.isArray(c.arguments) ? c.arguments[0] : undefined;
    const MODE_KO = { cool: '냉방', dry: '제습', wind: '송풍', fanOnly: '송풍', auto: '자동',
      heat: '난방', dryClean: '제습청정', coolClean: '냉방청정', aIComfort: 'AI쾌적' };
    switch (c.capability) {
      case 'switch':
        return `전원 → ${c.command === 'on' ? '켜짐' : c.command === 'off' ? '꺼짐' : c.command}`;
      case 'airConditionerMode':
        return `모드 → ${MODE_KO[arg] || arg}`;
      case 'thermostatCoolingSetpoint':
        return `설정온도 → ${arg}°C`;
      case 'custom.airConditionerOptionalMode':
        return `무풍 → ${arg === 'windFree' ? '켜짐' : '꺼짐'}`;
      case 'custom.autoCleaningMode':
        return `자동건조 → ${arg === 'on' ? '켜짐' : '꺼짐'}`;
      default: {
        const a = Array.isArray(c.arguments) && c.arguments.length ? `(${c.arguments.join(', ')})` : '';
        return `${c.capability}.${c.command}${a}`;
      }
    }
  }

  static _fmtCommands(commands) {
    return commands.map((c) => SmartThingsClient._fmtCommand(c)).join(', ');
  }

  async sendCommand(deviceId, command) {
    const commands = Array.isArray(command) ? command : [command];
    // v2.1.3 — 로그 포매터를 제어 경로 try 밖에서 선계산(감사 제안 ③): 포매터 예외가
    // 전송 성공을 실패로 둔갑시키지 못하게 한다. 현 호출자 기준 던질 경로는 없으나
    // (v2.1.1 감사에서 확인), 향후 동적 명령이 추가돼도 안전한 구조로.
    // 폴백의 JSON.stringify도 던질 수 있어(throwing getter 등 — R7(d) 실증) 이중 방어.
    let cmdDetail;
    try { cmdDetail = JSON.stringify(commands); }
    catch { cmdDetail = `(명령 ${commands.length}건 — 직렬화 불가)`; }
    let cmdSummary;
    try { cmdSummary = SmartThingsClient._fmtCommands(commands); }
    catch { cmdSummary = cmdDetail; }
    try {
      ...
      this.log.info(`[${this._labelOf(deviceId)}] 전송: ${cmdSummary}`);
      this.log.debug?.(`[전송 상세] ${deviceId} → ${cmdDetail}`);
      ...
      this.log.error(`[${this._labelOf(deviceId)}] 전송 실패: ${cmdSummary} — ${status || ''} ${e.message}`);
      // v2.1.3 — 실패 body를 debug→warn 재승격(감사 제안 ②): 실패가 실제 일어나는 순간
      // (밤 자동화 OFF 실패 등)이 원인 코드가 가장 필요한 시점인데 기본 레벨에서 안 보였다.
      // lib 전체에서 e.response?.data 캡처 지점은 여기뿐. 폭증 위험은 스트릭 억제가 이미 방어.
      if (body) {
        let bodyStr;
        try { bodyStr = JSON.stringify(body); } catch { bodyStr = String(body); }
        this.log.warn(`[전송 실패 상세] ${this._labelOf(deviceId)}: ${bodyStr}`);
      }
      throw e;
    }
  }
