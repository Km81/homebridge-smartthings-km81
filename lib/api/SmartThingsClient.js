...
class SmartThingsClient {
  ...
  async sendCommand(deviceId, command) {
    ...
    try {
      ...
      this.log.info(`[${this._labelOf(deviceId)}] 전송: ${SmartThingsClient._fmtCommands(commands)}`);
      this.log.debug?.(`[전송 상세] ${deviceId} → ${JSON.stringify(commands)}`);
      ...
      this.log.error(`[${this._labelOf(deviceId)}] 전송 실패: ${SmartThingsClient._fmtCommands(commands)} — ${status || ''} ${e.message}`);
      if (body) this.log.debug?.(`[전송 실패 상세] ${deviceId}: ${JSON.stringify(body)}`);
      throw e;
    }
  }
