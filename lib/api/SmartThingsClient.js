...
class SmartThingsClient {
  ...
  // 명령 배열을 사람이 읽는 한 줄로 — {capability,command,arguments} → "switch.off, mode.setMode(cool)"
  static _fmtCommands(commands) {
    return commands.map((c) => {
      const args = Array.isArray(c.arguments) && c.arguments.length ? `(${c.arguments.join(', ')})` : '';
      return `${c.capability}.${c.command}${args}`;
    }).join(', ');
  }
