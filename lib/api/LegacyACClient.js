...
class LegacyACClient {
      ...
      try {
        ...
        if (isNetworkError && attempt < retries && !yieldToWaiter) {
          ...
          if (this._failStreak === 0) {
            ...
            if (this.offlineIsNormal && connectError) this.log.debug(line);
  ...
  _logFinalFailure(e, attempt, yielded) {
    ...
    const offlineNoise = this.offlineIsNormal
      && /ETIMEDOUT|ECONNRESET|ECONNREFUSED|EHOSTUNREACH|ENOTFOUND|EPIPE|ENETUNREACH/.test(e.message);
    if (offlineNoise) {
