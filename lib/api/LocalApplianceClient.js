...
const MODE_MAP = {
  cool: 'Cool',
  dry: 'Dry',
  dryclean: 'Dry',       // 제습청정(클라우드 전용 이름) → 로컬은 Dry로 근사. §미검증
  wind: 'Fan',
  fan: 'Fan',
  auto: 'AIComfort',
  aicomfort: 'AIComfort',
...
class LocalApplianceClient {
  ...
  async _ensureDeps() {
    const marker = path.join(this.libDir, 'smartthings_local');
    ...
    await new Promise((resolve, reject) => {
      ...
      p.on('exit', (code) => code === 0 ? resolve()
        : reject(new Error(`의존성 설치 실패 (코드 ${code})`)));
    });
  ...
  async _get(deviceId, segs, { fresh = false } = {}) {
    ...
    const res = await this._rpc({ op: 'get', host: d.host, port: d.port, path: segs, localPort: d.localPort });
  ...
  async _post(deviceId, segs, payload) {
    const d = this._dev(deviceId);
    const res = await this._rpc({ op: 'post', host: d.host, port: d.port, path: segs, payload, localPort: d.localPort });
  ...
  setMode(deviceId, mode) {
    ...
    if (!local) {
      this.log.warn(`[${this._labelOf(deviceId)}] 로컬 모드 매핑 없음(${mode}) — 전송 생략`);
      return Promise.resolve();
    }
    return this._withFallback(deviceId, '모드 전송',
      () => this._post(deviceId, ['mode', 'vs', '0'], { 'x.com.samsung.da.modes': [local] }),
