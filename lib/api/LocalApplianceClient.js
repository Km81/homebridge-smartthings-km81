...
// ★v2.14.11 — 최후 안전망. 지금까지 브릿지 재시작은 **프로세스가 죽었을 때만** 일어났고,
//   "프로세스는 살아 있는데 기기와 못 붙는" 상태는 아무도 손대지 않아 **사람이 홈브릿지를
//   재기동할 때까지 영원히** 그대로였다(2026-08-13 정수기: 4시간 39분 무통신 → 재기동 8초 만에 복구).
//   파이썬 쪽 세션 해제(bridge.py COAP_FAIL_DROP_AFTER)로도 안 풀리는 경우의 마지막 수단이다.
//   ⚠️다른 기기 세션도 함께 끊기지만 다음 요청에서 각자 재연결한다. 그래서 임계를 충분히 크게
//   둔다 — 60초 폴 기준 30회 ≈ 30분.
const BRIDGE_RESTART_AFTER = 30;
// ⚠️v2.14.12 — v2.14.11 은 임계를 **요청 단위 streak** 에만 걸어 두 방향으로 틀렸다(적대 리뷰 2축).
//   ①발화 조건이 `_fallbackStreak` 라 **정수기는 아예 못 셌다** — 8/13 사고 당사자가 안전망 밖이었다
//     (정수기 getter 는 `_withFallback` 을 안 타고 `_get` 직결이라 그 계정을 하나도 올리지 않는다).
//   ②반대로 폴이 빠른 기기에서는 예상보다 자주 발화하고 **멈출 조건이 없었다** — 영구 사망 기기가
//     하나 있으면 영원히 반복하며 그때마다 **다른 기기의 in-flight 쓰기가 유실**된다
//     (kill 시 `로컬 브릿지가 종료됨` 이 결과불명으로 분류돼 클라우드 재전송까지 금지된다).
//   → 시간 쿨다운과 횟수 상한을 함께 둔다. 로컬이 한 번이라도 성공하면 상한은 리셋된다.
const BRIDGE_RESTART_COOLDOWN_MS = 30 * 60 * 1000;
const BRIDGE_RESTART_MAX = 6;
...
class LocalApplianceClient {
  constructor(log, opts = {}) {
    ...
    this._readyWaiters = [];
    this._restarts = 0;
    // deviceId → 안전망 재시작 횟수. ★기기별이다 — 전역 하나로 두면 정상 기기의 성공이
    //   죽은 기기의 카운터를 지워 상한이 무한이 된다(v2.14.12 실사고).
    this._bridgeRestartsBy = new Map();
    this._stopped = false;
  }
  ...
  // ★v2.14.11 — "프로세스는 살아 있는데 기기와 못 붙는" 상태의 마지막 탈출구.
  //   kill 만 하고 여기서 직접 띄우지 않는다 — exit 핸들러가 백오프 후 _spawn() 하므로
  //   여기서 또 부르면 이중 spawn 이 된다. ready 되면 _restarts 가 0으로 돌아가 백오프도 리셋된다.
  // ★v2.14.12 — 임계 판정까지 여기서 한다. 호출부(홈킷 경로·정수기 폴러)가 각자 조건을 쓰면
  //   한쪽에만 연결되는 사고가 또 난다(v2.14.11 이 정확히 그랬다). 판정을 한 곳에 모은다.
  //   반환값 = 실제로 재시작을 시도했는지(테스트가 행위로 잴 수 있게).
  maybeRestartBridge(deviceId, streak, label) {
    if (!Number.isFinite(streak) || streak < BRIDGE_RESTART_AFTER) return false;
    if (this._stopped) return false;
    if (!this._proc) return false;   // 이미 죽어 있으면 exit 경로가 재시작을 맡고 있다
    const now = Date.now();
    // 시간 쿨다운 — streak 이 요청 단위라 폴이 빠른 기기에서 임계가 금방 다시 찬다
    if (now - (this._lastBridgeRestart || 0) < BRIDGE_RESTART_COOLDOWN_MS) return false;
    // 횟수 상한 — 기기가 물리적으로 죽어 있으면 재시작으로 안 고쳐진다. 영원히 반복하면
    // 애먼 기기의 세션만 30분마다 끊는다. **그 기기의** 로컬 성공 1회로 리셋된다(_noteLocalOk).
    // ★v2.14.13 — 상한은 **기기별**이다. v2.14.12 는 전역 카운터 하나였는데 리셋을
    //   `_noteLocalOk()` 가 **어느 기기의 성공에서든** 걸어 버려, 정상 기기(에어컨·건조기)가
    //   매 폴마다 0으로 되돌렸다 → 8/15 실사고에서 재시작이 4회 도는 내내 로그가 `1/6회째`.
    //   **상한이 사실상 무한**이었다는 뜻이고, 영구 사망 기기가 있으면 30분마다 영원히
    //   애먼 기기 세션을 끊는다(적대 리뷰가 경고한 바로 그 상태였다).
    // ⚠️생성자를 안 거치는 인스턴스(테스트 하네스 등)에서도 죽지 않게 지연 생성한다 —
    //   v2.14.13 개발 중 `getter_contract` 가 정확히 이 TypeError 로 깨졌다.
    if (!this._bridgeRestartsBy) this._bridgeRestartsBy = new Map();
    const done = this._bridgeRestartsBy.get(deviceId) || 0;
    if (done >= BRIDGE_RESTART_MAX) return false;
    this._lastBridgeRestart = now;
    this._bridgeRestartsBy.set(deviceId, done + 1);
    const nm = label || this._labelOf(deviceId);
    // ⚠️문구는 사실만 — 발화 조건은 "기기가 응답하는지"를 구분하지 못한다(적대 리뷰 D5).
    //   기기 자체가 꺼져 있어도 같은 경로로 온다.
    this.log.warn(`[${nm}] 로컬 연속 실패 ${streak}회 — 로컬 브릿지를 다시 시작합니다 `
      + `(${done + 1}/${BRIDGE_RESTART_MAX}회째). `
      + `세션만 죽어 못 붙는 경우를 위한 마지막 수단이며, 기기 전원이 꺼져 있으면 이걸로는 복구되지 않습니다`);
    try { this._proc.kill(); } catch (_) { /* 이미 종료됨 */ }
    return true;
  }

  // 로컬이 한 번이라도 성공하면 재시작 상한을 리셋한다 — 다음 장애를 다시 6회까지 구제한다.
  _noteLocalOk(deviceId) {
    // ⚠️deviceId 없이 부르면 안 된다 — 전역 리셋이 곧 v2.14.12 의 결함이었다.
    if (deviceId !== undefined && this._bridgeRestartsBy) this._bridgeRestartsBy.delete(deviceId);
  }
  ...
  async probeIdentity(host, port, localPort) {
    ...
    this._stat(deviceId).lastOk = Date.now(); this._noteLocalOk(deviceId);   // 신원 확인도 실통신이다(적대 리뷰 F7)
  ...
  async _get(deviceId, segs, { fresh = false } = {}) {
    ...
    this._stat(deviceId).lastOk = Date.now(); this._noteLocalOk(deviceId);
    ...
    return this._serialize(deviceId, async () => {
      try {
        ...
        if (!permanent && streak === LOCAL_DEAD_AFTER) {
          ...
          // ★v2.14.11 — 마지막 실패 사유를 함께 남긴다. 이게 없어서 2026-08-13 정수기 사고 때
          //   "기기가 응답을 안 한 것(예외)"인지 "응답은 하는데 오류를 준 것(CoAP 4.xx/5.xx)"인지
          //   로그만으로 가를 수 없었다 — 조치가 정반대인데 근거가 없었다.
          const why = (e && e.message) ? ` (마지막 실패: ${e.message})` : '';
          this.log.error(canFallback
            ? `[${this._labelOf(deviceId)}] 로컬 경로가 계속 실패해 사실상 클라우드로 동작 중입니다 — 브릿지/기기 확인 필요${why}`
            : `[${this._labelOf(deviceId)}] 로컬 경로가 ${streak}회 연속 실패했고 클라우드 폴백도 꺼져 있습니다 — 이 기기는 지금 제어되지 않습니다. 브릿지/기기/IP를 확인하세요${why}`);
        }

        // ★v2.14.11 최후 안전망 / v2.14.12 판정을 maybeRestartBridge 로 일원화
        if (!permanent) this.maybeRestartBridge(deviceId, streak);
