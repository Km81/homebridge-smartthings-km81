...
class LocalApplianceClient {
  ...
  maybeRestartBridge(deviceId, streak, label) {
    ...
    // 애먼 기기의 세션만 30분마다 끊는다. 로컬 성공 1회로 리셋된다(_noteLocalOk).
    if ((this._bridgeRestarts || 0) >= BRIDGE_RESTART_MAX) return false;
    this._lastBridgeRestart = now;
    this._bridgeRestarts = (this._bridgeRestarts || 0) + 1;
    ...
    this.log.warn(`[${nm}] 로컬 연속 실패 ${streak}회 — 로컬 브릿지를 다시 시작합니다 `
      + `(${this._bridgeRestarts}/${BRIDGE_RESTART_MAX}회째). `
  ...
  _noteLocalOk() {
    if (this._bridgeRestarts) this._bridgeRestarts = 0;
  }
  ...
  async probeIdentity(host, port, localPort) {
    ...
    this._stat(deviceId).lastOk = Date.now(); this._noteLocalOk();   // 신원 확인도 실통신이다(적대 리뷰 F7)
  ...
  async _get(deviceId, segs, { fresh = false } = {}) {
    ...
    this._stat(deviceId).lastOk = Date.now(); this._noteLocalOk();
