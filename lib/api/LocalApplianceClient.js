...
class LocalApplianceClient {
  ...
  _serialize(deviceId, fn) {
    if (this._inChain.get(deviceId)) return Promise.resolve().then(fn);
    const run = async () => {
      this._inChain.set(deviceId, true);
      try {
        return await fn();
      } finally {
  ...
  async _doVerify(deviceId) {
    ...
    const res = await this._rpc({ op: 'get', host: d.host, port: d.port, path: ['oic', 'd'], localPort: d.localPort });
  ...
  async _withFallback(deviceId, label, localFn, cloudFn, opts = {}) {
    ...
    return this._serialize(deviceId, async () => {
      try {
        ...
        const d = this.devices.get(deviceId);
        const unknown = /시간 초과|종료됨/.test(e.message || '');
        ...
        if (streak === LOCAL_DEAD_AFTER) {
          this.log.error(`[${this._labelOf(deviceId)}] 로컬 경로가 계속 실패해 사실상 클라우드로 동작 중입니다 — 브릿지/기기 확인 필요`);
        }

        const canFallback = this.cloud && d?.fallbackToCloud !== false && typeof cloudFn === 'function';
        if (canFallback && unknown && !fallbackOnUnknown) {
