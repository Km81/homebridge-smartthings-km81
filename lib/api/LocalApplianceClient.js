...
class LocalApplianceClient {
  ...
  _restartBridge(deviceId, streak) {
    if (this._stopped) return;
    if (!this._proc) return;   // 이미 죽어 있으면 exit 경로가 재시작을 맡고 있다
    this.log.warn(`[${this._labelOf(deviceId)}] 로컬 연속 실패 ${streak}회 — 로컬 브릿지를 다시 시작합니다`
      + ` (기기는 응답하는데 세션만 죽은 상태에서 빠져나오는 마지막 수단입니다)`);
    try { this._proc.kill(); } catch (_) { /* 이미 종료됨 */ }
  }
  ...
  async _withFallback(deviceId, label, localFn, cloudFn, opts = {}) {
    ...
    return this._serialize(deviceId, async () => {
      try {
        ...
        // ★v2.14.11 최후 안전망 — 세션 해제로도 안 풀리면 브릿지 프로세스를 다시 띄운다.
        //   `>=` + 나머지 판정이라 30·60·90회에서 반복 시도한다(`===` 로 두면 한 번 놓치고
        //   그 뒤로는 영영 안 돈다 — 이 저장소에서 이미 밟은 부류다).
        if (!permanent && streak >= BRIDGE_RESTART_AFTER && streak % BRIDGE_RESTART_AFTER === 0) {
          this._restartBridge(deviceId, streak);
        }
