...
class LocalApplianceClient {
  ...
  async _ensureDeps() {
    ...
    const runPip = (extraArgs) => new Promise((resolve) => {
      const args = ['-m', 'pip', 'install', '--target', this.libDir,
    ...
    } else if (/Temporary failure in name resolution|Could not find a version|Network is unreachable|timed out|ProxyError/i.test(r.out)) {
