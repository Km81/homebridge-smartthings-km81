...
class LocalApplianceClient {
  ...
  _forgetDiscovered(host) {
    ...
    try {
      ...
      delete all[host];
      fs.writeFileSync(this._discoveredPath(), JSON.stringify(all, null, 2));
  ...
  async _doVerify(deviceId) {
    ...
    await this._dumpResourcesOnce(deviceId);
  }
    ...
    return this._serialize(deviceId, async () => {
      try {
        ...
        if (canFallback) {
          ...
          if (permanent) {
            // ★리소스 부재는 복구되지 않는다 — 폴백이 켜져 있으면 **폴링마다 클라우드 호출**이
            //   된다. 연속 실패로 세지 않으니 기존 경고가 전부 침묵해 조용한 과금이 된다.
            //   상태이지 사건이므로 반복하지 않고, 기기·항목당 한 번만 알린다.
            const key = `${deviceId}|${label}`;
            if (!this._permanentNotified.has(key)) {
              this._permanentNotified.add(key);
              this.log.warn(`[${this._labelOf(deviceId)}] 이 기기에 ${label} 리소스가 없어 앞으로 폴링마다 클라우드를 부릅니다`);
            }
          } else if (!this._fallbackSince.has(deviceId)) {
  ...
  static _noCloudWind() {
    throw new Error('바람방향은 클라우드로 제어할 수 없습니다 — 전송 경로를 로컬로 두세요');
  }
  ...
  async getWindDirection(deviceId) {
    return this._withFallback(deviceId, '바람방향 조회',
      ...
      LocalApplianceClient._noCloudWind);
  }
