...
class LocalApplianceClient {
  ...
  _stat(deviceId) {
    ...
    if (!st) {
      st = { ok: 0, fail: 0, cmdFail: 0, cloud: 0, outages: 0, longestMs: 0, since: Date.now() };
  ...
  async getDryerCourse(deviceId) {
    try {
      ...
      const hit = opts.find((o) => typeof o === 'string' && /^Course_/.test(o));
      if (!hit) return null;
   ...
   *  ⚠️`sleepTime` 은 `HHMMHHMM`(시작·종료) 로 **보이지만 미검증**이다. 원문을 그대로 넘긴다. */
  async getAiSleep(deviceId) {
    try {
      const r = await this._get(deviceId, ['aisleep', 'vs', '0']);
      const night = r?.['x.com.samsung.da.displayNightMode'];
      if (typeof night !== 'string' || !night) return null;
      const el = Number(r?.['x.com.samsung.da.elapsedTime']);
      return {
        night_mode: night === 'On',
        elapsed_min: Number.isFinite(el) ? el : null,
        sleep_time: r?.['x.com.samsung.da.sleepTime'] ?? null,   // 원문 그대로(형식 미검증)
        status: r?.['x.com.samsung.da.statusFeedback'] ?? null,
      };
    } catch (e) { return null; }
  }
  ...
  async getFavoriteCapacity(deviceId) {
    try {
      ...
      const def = Number(r?.['x.com.samsung.da.defaultCapacity']);
      return {
        enabled: sw === 'On',
