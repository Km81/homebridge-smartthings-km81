    ...
    const poll = async () => {
      ...
      try {
        ...
        for (const unit of units) {
          ...
          let currentState = combineStates(
            ...comps.map(c => (c ? classifyComponent(c) : STATE.UNKNOWN))
          );
          ...
          if (currentState === STATE.UNKNOWN && previousState && previousState !== STATE.UNKNOWN) {
            ...
            if (unit.unknownCarry > MAX_UNKNOWN_CARRY) {
              this.log.warn(`[${unit.label}] 상태 정보가 ${unit.unknownCarry}회 연속 비어있어 FINISHED로 강제 전환합니다 (가전 오프라인 의심).`);
          ...
          this.log.debug(
            `[${unit.label}] 폴링: state=${currentState} (prev=${previousState ?? 'null'}), ` +
