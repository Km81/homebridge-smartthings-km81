...
const LegacyLaundryClient = require('../api/LegacyLaundryClient');
...
// ★v2.14.14 — 로컬 무응답을 '꺼짐'으로 단정하기까지 요구하는 연속 실패 횟수.
//   세탁기(8888 클라이언트)의 OFFLINE_AFTER 와 같은 값·같은 사상이다.
//   ⚠️직전에 **돌고 있었으면** 이만큼 요구한다 — 순단을 종료로 오인하면 거짓 종료 알림이 나간다.
const LOCAL_OFFLINE_AFTER = 5;

// ★v2.14.16 — 판정을 **순수 함수**로 뺀다. 홈킷 경로(Laundry)와 MQTT 중계 경로(attach)가
//   같은 로직을 써야 한쪽만 고치는 사고가 안 난다 — v2.14.15 는 홈킷만 고쳐 **HA 는 그대로
//   "운전 중 61%" 에 고착**돼 있었다(2026-08-19 실측).
// ⚠️`_transient` 로 거르지 않는다: 두 전송 계층이 같은 이름을 다른 뜻으로 쓴다(v2.14.15).
function buildSyntheticOff({ err, streak, wasRunning, unitCount, enabled }) {
  if (!enabled) return null;
  if (err && err._noToken) return null;
  if (!CONNECTION_ERROR_RE.test(String((err && err.message) || ''))) return null;
  if (wasRunning && streak < LOCAL_OFFLINE_AFTER) return null;
  const off = {};
  const NAMES = ['main', 'sub'];
  const count = Math.min(Math.max(1, unitCount || 1), NAMES.length);
...
class Laundry {
  ...
  configure(accessory, configDevice, packageVersion) {
    ...
    this.units = [];
    // ★이 기기가 만든 액세서리 전부(본체 + 보조 세탁조 + 종료알림 센서) — 임시 연결해제 봉인 대상.
    //   ⚠️2.16.0 은 `u.accessory` 를 읽었는데 유닛에 그런 필드가 **원래 없었다**(리뷰 실측) —
    //   그래서 이름으로 추측하지 않고, 만드는 자리에서 직접 담는다.
    this._ownedAccessories = [accessory];
    ...
    // ★「임시 연결해제」 — 유닛·서비스는 다 만든 뒤에 **폴링만** 안 건다.
    if (isPutAway(configDevice)) {
      this._stopped = true;
      const sealed = sealForPutAway(this._ownedAccessories);
  ...
  _ensureAccessory(device, keySuffix, displayName, configDevice, packageVersion) {
    ...
    if (this._ownedAccessories) this._ownedAccessories.push(acc);
  ...
  // ★v2.14.14 — 건조기(로컬 DTLS)는 무응답이 **예외**로 올라와 폴 catch 가 직전 상태를 그대로
  //   둔다. 그래서 다 돌고 꺼져 네트워크에서 사라져도 홈킷이 **"작동 중"에 고착**됐다
  //   (2026-08-19 사용자 신고: SmartThings 는 오프라인인데 홈킷은 작동 중).
  //   세탁기(8888)는 클라이언트가 합성 '꺼짐'을 만들어 이 문제가 없다 —
  //   **사용자 요청으로 같은 판정을 여기서 한다**(세탁기와 동일 동작).
  //
  //   조건 없이 만들면 안 된다:
  //    · 토큰이 있는 기기(세탁기)는 클라이언트가 이미 처리하므로 여기 오지 않는다(게이트로 재확인).
  //    · 네트워크류 무응답만 — 응답 구조 이상·인증 문제는 진짜 오류다.
  //    · ⚠️직전에 운전 중이었으면 연속 N회를 요구한다. 순단(건조기 실측 최장 29초)을 종료로
  //      오인하면 **거짓 종료 알림**이 나간다.
  //   반환값에 __synthetic 을 달아 last_seen·접속선이 발화하지 않게 한다(세탁기와 동일 규약).
  _syntheticOffFor(e, streak, wasRunning, unitCount) {
    return buildSyntheticOff({
      err: e, streak, wasRunning, unitCount, enabled: this._localSyntheticOff,
    });
  }
  ...
  _startPolling(accessory, configDevice, units) {
    ...
    // ★v2.14.14 — 합성 '꺼짐' 게이트. **로컬 전송인데 토큰이 없는 기기**(= DTLS 신형, 건조기)만이다.
    //   토큰이 있는 세탁기는 LegacyLaundryClient 가 이미 같은 일을 하므로 예외가 여기까지 안 온다.
    //   ⚠️게이트를 안 세우면 클라우드 경로 기기까지 무응답을 '꺼짐'으로 단정한다.
    const _lc = configDevice && configDevice.local;
    this._localSyntheticOff = !!(_lc && _lc.host && !_lc.token);
    ...
    const poll = async () => {
      ...
      try {
        ...
        let components;
        try {
          components = await this.smartthings.getStatus(deviceId);
          this._synthOffAnnounced = false;   // 실제 응답이 왔으면 다음 무응답에 다시 알린다
        } catch (e) {
          const wasRunning = units.some(u => u.prev === STATE.RUNNING || u.prev === STATE.PAUSED);
          const off = this._syntheticOffFor(e, consecutiveFailures + 1, wasRunning, units.length);
          if (!off) throw e;
          components = off;
          if (!this._synthOffAnnounced) {
            this._synthOffAnnounced = true;
            // ⚠️'실패/오류'가 아닌 문구 — 전원을 끈 것은 고장이 아니고 hb-watch 경보에 걸리면 안 된다
            //   (세탁기와 같은 문구를 쓴다).
            this.log.info(`[${baseLabel}] 전원 꺼짐 — 로컬 응답 없음`);
          }
        }
        ...
        // ★MQTT 중계용 훅 — 이 기기는 **로컬 폴 경로가 없어**(8888/클라우드) 브리지가 스스로
        //   생사를 알 수 없다. 폴이 성공한 시각을 알려 주는 것이 유일한 방법이다.
        //   ⚠️Valve 특성 미러로는 대신할 수 없다 — 값이 안 바뀌면 이벤트가 안 오므로
        //   "폴은 성공했는데 값이 그대로"가 "죽었다"로 오판된다(막으려는 바로 그것).
        // ⚠️합성 응답(기기 무응답 → 우리가 만든 '꺼짐')에는 발화하지 않는다 — 그러면
        //   닿지도 않은 기기의 `last_seen` 이 계속 전진해 **어떤 장애에도 사망 판정이
        //   발화하지 않는다**(적대 리뷰 F5).
        if (typeof this._onPollOk === 'function' && !(components && components.__synthetic)) {
          try { this._onPollOk(); } catch (e) {}
        }
        // ★재시작 구멍(2026-08-05 적대 리뷰) — LegacyAC 와 같은 이유. 실패 카운터가 메모리에만
        //   있어 🔴 뒤 재시작하면 복구 어휘가 영영 안 나온다. 첫 성공에 한 번만 접촉선을 남긴다.
        // ⚠️같은 이유로 합성 응답에는 접촉선을 남기지 않는다 — 기기에 한 번도 닿지 않았는데
        //   복구 어휘가 나가면 감시기의 진짜 경보를 잘못 푼다(적대 리뷰 F4).
        if (!this._contactAnnounced && !(components && components.__synthetic)) {
          this._contactAnnounced = true;
          this.log.info(`[${baseLabel}] 기기 접속됨 — 상태 조회 정상`);
        }
...
module.exports.buildSyntheticOff = buildSyntheticOff;
