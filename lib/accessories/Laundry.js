...
class Laundry {
  ...
  _syntheticOffFor(e, streak, wasRunning, unitCount) {
    if (!this._localSyntheticOff) return null;
    if (e && e._noToken) return null;
    // ⚠️v2.14.15 — `_transient` 로 거르면 안 된다. 두 전송 계층이 **같은 이름을 다른 뜻으로** 쓴다:
    //   · 세탁기(8888)  : "아직 꺼짐으로 단정할 단계가 아니다"(직전 상태 유지 신호)
    //   · 건조기(DTLS) : `e._transient = kind === 'read'` — 그냥 **읽기 실패였다**는 표식이라
    //                     모든 읽기 실패에 붙는다.
    //   v2.14.14 는 세탁기 뜻으로 읽고 제외했다가 **건조기에서 기능이 통째로 사문**이었다
    //   (실측 진단: `off=null transient=true reMatch=true`). 순단 보호는 아래 두 줄이 맡는다.
    if (!CONNECTION_ERROR_RE.test(String((e && e.message) || ''))) return null;
