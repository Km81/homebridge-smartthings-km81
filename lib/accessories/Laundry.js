...
class Laundry {
  ...
  _setupValveService(accessory) {
    ...
    this._bindCharacteristic({
      ...
      getter: async () => {
        const s = this._state.state;
        return (s === STATE.RUNNING || s === STATE.PAUSED) ? this._state.duration : 0;
      },
    });
    this._bindCharacteristic({
      service,
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (currentState === STATE.UNKNOWN && previousState && previousState !== STATE.UNKNOWN) {
          unknownCarryCount++;
          if (unknownCarryCount > MAX_UNKNOWN_CARRY) {
            this.log.warn(`[${baseLabel}] 상태 정보가 ${unknownCarryCount}회 연속 비어있어 FINISHED로 강제 전환합니다 (가전 오프라인 의심).`);
        ...
        if (previousState && previousState !== currentState) {
          this.log.info(`[${baseLabel}] 상태 → ${stateKo(currentState)} (이전 ${stateKo(previousState)})`);
        }
