...
class Laundry {
  ...
  _setupValveService(accessory) {
    ...
    // SetDuration: 모니터링 전용이라 사용자 설정은 무시하고 현재 잔여 시간을 반환
    const setDurChar = service.getCharacteristic(C.SetDuration);
    setDurChar.removeAllListeners('get');
    setDurChar.removeAllListeners('set');
    setDurChar.on('get', (cb) => cb(null, this._state.active ? this._state.duration : 0));
    setDurChar.on('set', (_v, cb) => cb(null));
