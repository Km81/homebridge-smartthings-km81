...
function isComponentActive(component) {
  ...
  // 명시적 종료 jobState는 machineState가 어떻든 비활성.
  if (jobState && FINISHED_JOB_STATES.has(jobState)) return false;
  // jobState가 보고되면 그것이 진실의 원천. machineState='on'(대기)는 무시.
  if (jobState) return ACTIVE_JOB_STATES.has(jobState);
  // jobState가 없는 모델만 machineState로 판정 ('run'만 active, 'on'은 대기).
  return ACTIVE_MACHINE_STATES.has(machineState);
}
