...
class Laundry {
  ...
  _setupValveService(accessory) {
    ...
    // 이전 버전(v1.5.3/1.5.5)에서 추가됐을 수 있는 SetDuration 특성을 제거.
    // (Home 앱은 SetDuration이 있으면 '기본 작동 시간' 슬라이더를 강제로 노출함)
    if (service.testCharacteristic(C.SetDuration)) {
      service.removeCharacteristic(service.getCharacteristic(C.SetDuration));
    }
