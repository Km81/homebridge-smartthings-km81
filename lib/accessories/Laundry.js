...
function isComponentActive(component) {
  ...
  if (!jobState && !machineState) return false;

  // 명시적 종료 jobState는 어떤 경우에도 비활성으로 본다.
  if (jobState && FINISHED_JOB_STATES.has(jobState)) return false;

  // v1.8.4 정정: 실제 사용자 로그(`job=weightSensing machine=stop`) 분석 결과,
  // 일부 Samsung 모델은 운전 종료 후에도 jobState를 'weightSensing' 같은 운전 시작 전
  // 단계로 stale하게 남긴다. 반면 machineState는 'stop'으로 정확히 보고된다.
  // 따라서 machineState가 보고되면 그것을 진실의 원천으로 삼고, 'run'만 active로 본다.
  if (machineState) return ACTIVE_MACHINE_STATES.has(machineState);

  // machineState가 보고되지 않는 모델만 jobState로 폴백.
  return ACTIVE_JOB_STATES.has(jobState);
}
  ...
  _startPolling(accessory, configDevice, valveService, sensorService) {
    ...
    const poll = async () => {
      ...
      try {
        ...
        // 진단용: 종료 후에도 active가 풀리지 않는 모델을 추적할 수 있도록 원본 상태 값을 함께 출력.
        const mainStates = _readJobAndMachineState(mainComp);
        const subStates = _readJobAndMachineState(subComp);
        this.log.debug(
          `[${baseLabel}] 폴링: active=${active}, remaining=${rawDuration}s (cap ${duration}s) ` +
        ...
        // SetDuration은 매 poll re-assert. (외부 자동화/Siri가 잘못 쓴 값을 즉시 덮어쓰도록.)
        if (active) {
