...
class Laundry {
  ...
  _setupValveService(accessory) {
    ...
    // 앱 첫 진입 시 즉답용 get 핸들러 (실시간 갱신은 _startPolling의 push 업데이트가 담당)
    const getStatus = async () => {
      ...
      const seconds = Math.max(getComponentDuration(mainComp), getComponentDuration(subComp));
      return { active, duration: Math.min(seconds, HK_REMAINING_DURATION_MAX) };
    };

    this._bindCharacteristic({
      service,
      characteristic: C.Active,
      getter: async () => (await getStatus()).active ? C.Active.ACTIVE : C.Active.INACTIVE,
    });
    this._bindCharacteristic({
      service,
      characteristic: C.InUse,
      getter: async () => (await getStatus()).active ? C.InUse.IN_USE : C.InUse.NOT_IN_USE,
    });
    this._bindCharacteristic({
      service,
      characteristic: C.RemainingDuration,
      getter: async () => {
        const { active, duration } = await getStatus();
