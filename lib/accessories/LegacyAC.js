...
class LegacyAC {
  ...
  _scheduleOffAbsorbVerify() {
    ...
    this._offVerifyTimer = setTimeout(async () => {
      ...
      if (this._offIntentTs === 0 || this._offIntentTs !== intentTs) return; // ON/새 off가 끼어듦
      try {
        await this.getCachedState(true); // 공유 캐시 우회 강제 실측
        // in-flight GET에 합류해 의도 이전의 스냅샷을 받았을 수 있음 — 실측 시각이 의도보다
        // 오래됐으면 1회 재실측(그래도 오래면 fail-safe로 이번 검증은 건너뜀 = v1.8.27 동작).
        if (!(this._lastFetchTs > intentTs) && !this._stopped) {
          await this.getCachedState(true);
        }
        if (this._stopped) return;
        if (this._offIntentTs === 0 || this._offIntentTs !== intentTs) return;
        if (!(this._lastFetchTs > intentTs)) return;
        ...
        this.debugLog(`끄기 흡수 검증 실패(무해 — 다음 폴이 보정): ${e.message}`);
      }
    }, 2000);
  }
