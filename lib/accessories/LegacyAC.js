...
class LegacyAC {
  ...
  _openOnGuard() {
    ...
    this.debugLog(`ON 보호 윈도우 시작 (${this._onGuardMs}ms, 전략=${this._onGuardStrategy})`);
  ...
  _closeOnGuard(reason) {
    ...
    this._onGuardUntil = 0;
    this.debugLog(`ON 보호 윈도우 종료 (${reason})`);
  ...
  // 항상 _patchState로 in-memory를 갱신하므로 HomeKit UI는 안정.
  async _sendUnlessOnGuard(endpoint, data, patchFn) {
    ...
    if (this._isOnGuardActive()) {
      ...
      this.debugLog(`ON 보호 중 명령 보류: ${endpoint} -> ${JSON.stringify(data)}`);
      return;
    }
  ...
  setupCharacteristics() {
    ...
    // 슬라이더 드래그: trailing-debounce(400ms) + 즉시 _patchState + 보호 윈도우 인지
    ...
    const flushTempSet = async () => {
      ...
      // 같은 값이면 명령 생략 (idempotency)
      const currentDesired = this.deviceState?.Temperatures?.[0]?.desired;
      if (currentDesired === value) {
