...
class LegacyAC {
  constructor({ log, config, api, accessory, packageRoot }) {
    ...
    // v1.3.0: only one HK mode (cool). Heat/Auto removed entirely.
    ...
    this.log.info(`[${this.name}] LegacyAC 초기화 완료. (Cool 모드 = ${this.coolModeStr || '(없음)'})`);
  }

  _resolveCoolMode() {
    const cfg = this.config;
    if (typeof cfg.hkCoolMode === 'string' && cfg.hkCoolMode.length > 0 && cfg.hkCoolMode !== 'none') {
      return cfg.hkCoolMode;
    }
  ...
  setupCharacteristics() {
    ...
    // v1.3.0: Active just sends power. Mode application handled by TargetState.
    s.getCharacteristic(C.Active)
      ...
      .onSet(async (value) => {
        ...
        } catch (e) { this.log.error(`SET Active 오류:`, e.message); throw e; }
      });

    // CurrentHeaterCoolerState: only INACTIVE / IDLE / COOLING (no HEATING).
    ...
    // TargetHeaterCoolerState: only COOL.
    s.getCharacteristic(C.TargetHeaterCoolerState)
      ...
      .onSet(async (value) => {
        ...
        try {
          ...
          await this._refreshState();
        } catch (e) { this.log.error(`SET TargetState 오류:`, e.message); throw e; }
      });
