...
const CONSTANTS = {
  ...
  // v1.8.24 — 끄기 장면 형제 write 억제 창. 실측(승준): 장면 write는 off 후 ~1.2s 안에 모두
  // 도착(온도는 400ms 디바운스 추가). v1.8.26 — 2500→4000: 홈허브 지연으로 형제 write가 늦게
  // 도착하는 꼬리 케이스 마진 확보. ON 의도가 마커를 즉시 해제하므로 "끄고 바로 켜기"에는
  // 영향이 없고, 창이 억제하는 건 꺼진 직후의 형제 명령뿐이라 상향 비용이 사실상 0.
...
class LegacyAC {
  ...
  // v1.8.26 — 끄기 전송이 "실패"했을 때만 5초 뒤 1회 재시도(성공 경로는 건드리지 않음 — '띠' 원칙).
  ...
  _scheduleOffRetry() {
    ...
    this.log.warn(`[${this.name}] 끄기 전송 실패 — 5초 뒤 1회 재시도합니다.`);
    ...
    this._offRetryTimer = setTimeout(() => {
      ...
      this.sendCommand('', { Operation: { power: CONSTANTS.POWER.OFF } })
        ...
        .catch(e => this.log.warn(`[${this.name}] 끄기 재시도 실패 — 포기: ${e.message}`));
    }, 5000);
  }
  ...
  async getCachedState(force = false, maxAgeMs = 0) {
    const now = Date.now();
    ...
    this.stateRequestPromise = (async () => {
      try {
        ...
        const response = await this.client.getDeviceStatus(maxAgeMs);
