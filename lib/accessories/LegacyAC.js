...
class LegacyAC {
  constructor({ log, config, api, accessory, packageRoot }) {
    ...
    const guardLabel = this._onGuardMs > 0
      ...
      : '꺼짐';
    const resendLabel = this._resendModeOnPowerOn ? this.coolModeStr : '꺼짐';
  ...
  // v1.8.18 — 전원 ON 후속 모드 재전송. 보호 윈도우가 끝난 뒤 hkCoolMode를 1회 전송한다.
  // 주의: 여기서 deviceState 기반 모드 idempotency 검사를 하면 안 된다 — _sendUnlessOnGuard가
  // drop된 명령도 patchFn으로 deviceState를 선-patch하므로, 실제로 전송되지 않은 모드가
  // "이미 적용됨"으로 보여 재전송이 무산된다(이 기능이 잡으려는 바로 그 시나리오).
  _schedulePowerOnModeResend() {
    if (!this._resendModeOnPowerOn) return;
    if (!this.coolModeStr || this.coolModeStr === 'none') return;
    ...
    const fire = (retried) => {
      ...
      if (this.deviceState?.Operation?.power !== CONSTANTS.POWER.ON) {
        ...
        return; // 재확인에도 Off → 진짜 꺼짐, 취소
      }
      this.log.info(`[${this.name}] 전원 ON 후속 모드 재전송: ${this.coolModeStr}`);
      this.sendCommand('/mode', { modes: [this.coolModeStr] })
        .then(() => {
          this._patchState(st => {
            st.Mode = st.Mode || {};
        ...
        .catch(e => this.log.warn(`[${this.name}] 전원 ON 후속 모드 재전송 실패: ${e.message}`));
    };
    ...
    s.getCharacteristic(C.Active)
      ...
      .onSet(async (value) => {
        ...
        try {
          ...
          this._openOnGuard();
          this._schedulePowerOnModeResend();
          this._scheduleRefresh();
        } catch (e) {
