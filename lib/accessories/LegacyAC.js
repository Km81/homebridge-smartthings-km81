...
class LegacyAC {
  ...
  _buildHkPresets() {
    ...
    const resolve = (modeField, enabledField, modesField, defaults) => {
      ...
      return { enabled: true, modes: norm(cfg[modesField] ?? defaults.modes), options: [] };
    };
    return {
  ...
  shutdown() {
    ...
    if (this._powerOnTimer) clearTimeout(this._powerOnTimer);
  }
  ...
  async _applyPreset(presetName) {
    ...
    if (!preset) { this.log.warn(`[${this.name}] 알 수 없는 HK preset: ${presetName}`); return; }
    if (!preset.enabled) { this.debugLog(`Preset '${presetName}' 비활성`); return; }
    if (preset.modes.length > 0) await this.sendCommand('/mode', { modes: preset.modes });
    if (preset.options.length > 0) await this.sendCommand('/mode', { options: preset.options });
  }
  ...
  setupCharacteristics() {
    ...
    s.getCharacteristic(C.Active)
      ...
      .onSet(async (value) => {
        ...
        try {
          await this.sendCommand('', { Operation: { power: value ? CONSTANTS.POWER.ON : CONSTANTS.POWER.OFF } });
          if (value === 1) {
            // Defer powerOnMode application so a concurrent TargetState set
            // (HomeKit's typical Active=1 + TargetState=X batch) can mark
            // _lastTargetStateSetTs first. If TargetState fires within the
            // window, the deferred job skips so we don't overwrite the user's
            // mode selection.
            if (this._powerOnTimer) clearTimeout(this._powerOnTimer);
            this._powerOnTimer = setTimeout(async () => {
              this._powerOnTimer = null;
              const sinceLastTarget = Date.now() - this._lastTargetStateSetTs;
              if (this._lastTargetStateSetTs > 0 && sinceLastTarget < 3000) {
                this.debugLog(`powerOnMode 취소: TargetState ${sinceLastTarget}ms 전 변경됨`);
                return;
              }
              ...
              try {
                ...
                this.log.warn(`powerOnMode 적용 오류: ${e.message}`);
              }
            }, 800);
          }
      ...
      s.getCharacteristic(C.HeatingThresholdTemperature)
        ...
        .onSet(async (value) => {
          ...
          try {
            await this.sendCommand('/temperatures/0', { desired: value });
            await this._refreshState();
          } catch (e) { this.log.error(`SET HeatTargetTemp 오류:`, e.message); throw e; }
        });
    }
