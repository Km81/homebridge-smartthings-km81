...
const CONSTANTS = {
  ...
  // v2.14.9 — 후속 체인의 "기기가 정말 켜졌나" 확인 재시도. 횟수와 시간 두 상한을 함께 둔다.
  // 한 바퀴가 [강제 조회 + 간격]이라 조회가 느려지면(기기 무응답 시 DEFAULT_TIMEOUT_MS)
  // 횟수만으로는 총 대기가 얼마든 늘어난다 — 16회 × (5s + 1s) = 96초.
  // 횟수는 v1.8.22의 4회(≈9초)가 침실 실측에서 부족해 확대한 값이다.
  POWER_ON_CONFIRM_MAX_TRIES: 16,
  POWER_ON_CONFIRM_GAP_MS: 1000,
  POWER_ON_CONFIRM_BUDGET_MS: 30000,
...
class LegacyAC {
  ...
  _schedulePowerOnResends() {
    ...
    const fire = () => {
      ...
      // 보호창 대기는 확인 예산에 넣지 않는다(기기 응답을 기다린 시간이 아니다).
      if (this._isOnGuardActive()) {
        this._powerOnModeTimer = setTimeout(fire, Math.max(this._onGuardUntil - Date.now(), 0) + 100);
        return;
      }
      if (this.deviceState?.Operation?.power !== CONSTANTS.POWER.ON) {
