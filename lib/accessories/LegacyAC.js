...
class LegacyAC {
  ...
  _buildHkPresets() {
    ...
    return {
      cool: {
        enabled: cfg.hkCoolEnabled !== false,
        modes: norm(cfg.hkCoolModes ?? ['Cool']),
        options: norm(cfg.hkCoolOptions)
      },
      heat: {
        enabled: cfg.hkHeatEnabled === true,
        modes: norm(cfg.hkHeatModes),
        options: norm(cfg.hkHeatOptions)
      },
      auto: {
        enabled: cfg.hkAutoEnabled === true,
        modes: norm(cfg.hkAutoModes),
        options: norm(cfg.hkAutoOptions)
      }
    };
  }
    ...
    s.getCharacteristic(C.Active)
      ...
      .onSet(async (value) => {
        ...
        try {
          ...
          if (value === 1) {
            const powerOnMode = this.config.powerOnHkMode || 'none';
            if (powerOnMode !== 'none' && this.hkPresets[powerOnMode]?.enabled) {
              this.log.info(`[${this.name}] 전원 ON: '${powerOnMode}' 프리셋을 자동 적용합니다.`);
              await this._applyPreset(powerOnMode);
            }
          }
