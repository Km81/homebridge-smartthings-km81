...
const { isPutAway, PUT_AWAY_MESSAGE, PUT_AWAY_SEAL_EMPTY, sealForPutAway } = require('../common/putAway');
...
const CONSTANTS = {
  ...
  // v2.14.9/v2.14.10 — 후속 체인의 "기기가 정말 켜졌나" 확인 재시도. 횟수와 시간 두 상한을 함께 둔다.
  //
  // ★횟수의 근거(2026-08-11 실측): 침실 벽걸이가 3시간+ 꺼져 있다 켜졌을 때, On PUT 을 ACK 하고도
  //   상태 엔드포인트에 **명령 후 17.7초 시점까지 Off** 를 보고했고 32.7초 시점엔 On 이었다
  //   (HA 히스토리 소급 측정, 해상도 15초). v1.8.22 의 4회(≈9초)는 물론 v2.14.9 의 16회×1초
  //   (≈24초)로도 이 구간을 못 덮을 수 있어 20회×2초(보호창 포함 ≈48초)로 잡았다.
  // ★간격을 1초→2초로 늘린 이유: 주기 폴링이 10초마다 돌고 있어 매초 강제 조회는 과잉이다.
  //   같은 시간을 덮으면서 기기를 두드리는 횟수는 오히려 줄어든다.
  // ⚠️조회 1회는 5초가 아니다 — 클라이언트가 내부적으로 3attempt + 백오프(1s·2s)를 돌아
  //   **최대 ~18초**까지 쓴다(LegacyACClient `_requestWithRetry`). 그래서 횟수만으로는 총 대기가
  //   정해지지 않고(20회면 최악 ~380초), 시간 예산이 그 폭주를 막는다.
  // ⚠️예산은 재시도 **시작 직전**에 검사하는 소프트 상한이라, 마지막 조회 1사이클만큼 초과할 수
  //   있다(최악 60 + 18 + 2 ≈ 80초). 60초로 둔 것은 조회가 느릴 때 옛 4회 경로(최대 ~76초)보다
  //   크게 후퇴하지 않게 하려는 것이다.
  POWER_ON_CONFIRM_MAX_TRIES: 20,
  POWER_ON_CONFIRM_GAP_MS: 2000,
  POWER_ON_CONFIRM_BUDGET_MS: 60000,
  ...
  // 냉방 버튼에 매핑할 수 있는 모드. 설정 화면 선택지와 반드시 같아야 하므로
  // lib/shared.js 하나만 고친다(schema_ui가 코드↔스키마↔화면 일치를 기계로 확인).
  LEGACY_COOL_MODES: require('../shared.js').LEGACY_COOL_MODES,
...
class LegacyAC {
  constructor({ log, config, api, accessory, packageRoot }) {
    ...
    try {
      ...
      this.client = LegacyACClient.getShared(this.config.ip, this.config.token, this.log, {
        timeout: this.timeout, cert: certBuffer, key: keyBuffer, label: this.name
      });
    } catch (e) {
    ...
    // ★「임시 연결해제」 — 서비스를 다 만든 뒤에 **통신만** 끊는다(방 배치·장면은 그대로).
    //   ⛔액세서리를 지우지 않는다 · ⛔`_initialized` 는 true 로 둔다
    //     (false 로 두면 위쪽 플랫폼이 「초기화 실패」로 보고 홈킷에서 타일을 제거한다).
    if (isPutAway(this.config)) {
      this._stopped = true;        // ★이중 잠금 — getCachedState 가 새 네트워크 요청을 시작하지 않는다
      // ★홈킷에는 **마지막 상태를 정상 연결로** 보여 준다(게터를 떼면 HAP 이 복원된 값으로 답한다).
      //   ⛔이 줄이 없으면 게터가 던져 「응답 없음」이 된다 — 실제로 그랬다(v2.15.0).
      const sealed = sealForPutAway(this.accessory);
      this._initialized = true;
      this.log.info(`[${this.name}] ${PUT_AWAY_MESSAGE}`);
      if (sealed === 0) this.log.warn(`[${this.name}] ${PUT_AWAY_SEAL_EMPTY}`);
      return;                      // ⚠️아래 초기화 완료 로그도 내지 않는다 — 기동 로그는 이 한 줄뿐이어야 한다
    }
  ...
  _resolveCoolMode() {
    ...
    // ★기기가 스스로 보고하는 목록이 정본이다(v2.6.9).
    //   실기기 상태 덤프(거실·침실 2대, 2026-07-30) — 둘 다 아래 6개를 지원한다고 알려온다:
    //     Mode.supportedModes = ["Cool","Dry","Wind","CoolClean","DryClean","Auto"]
    //   그동안 4개만 노출해 송풍·자동을 고를 수 없었다. 다만 이 목록은 **모델마다 다를 수 있어**,
    //   기기가 목록을 알려주면 그것으로 다시 확인한다(_warnUnsupportedMode).
    const allowed = new Set(CONSTANTS.LEGACY_COOL_MODES);
  ...
  /**
   * 고른 냉방 모드를 기기가 실제로 지원하는지 확인한다(v2.6.9).
   * 지원 목록은 모델마다 다르다 — 우리 집 2대는 6개를 알려주지만 다른 집은 다를 수 있다.
   * 안 되는 모드를 골라 두면 냉방을 눌러도 아무 일이 없는데 그 이유가 어디에도 안 나왔다.
   * 기기가 목록을 알려줄 때만, 한 번만 짚어 준다.
   */
  _warnUnsupportedMode() {
    if (this._modeWarned || !this.coolModeStr || this.coolModeStr === 'none') return;
    const list = this.deviceState?.Mode?.supportedModes;
    if (!Array.isArray(list) || list.length === 0) return;
    this._modeWarned = true;
    if (list.includes(this.coolModeStr)) return;
    this.log.warn(`[${this.name}] 냉방 버튼에 설정한 '${this.coolModeStr}' 모드를 이 기기가 지원하지 않습니다`
      + ` — 지원 목록: ${list.join(', ')}. 설정에서 다른 모드를 고르세요.`);
  }
  ...
  shutdown() {
    ...
    if (this._onVerifyTimer) {
      clearTimeout(this._onVerifyTimer);
      this._onVerifyTimer = null;
    }
  ...
  // v2.1.2 — 실측 실패 시 재시도(아래 catch 참조). 지연은 시도별 2s→5s→15s.
  /**
   * ★켜기 흡수 검증 (v2.6.6 신설 — 끄기 쪽 검증과 대칭).
   *
   * "이미 켜져 있음"으로 ON을 생략했는데 실제로는 꺼져 있는 경우를 잡는다.
   * 끄기와 달리 **재전송하지 않고 알리기만** 한다 — 꺼진 기기에 ON을 자동 재전송하면
   * 사용자가 원치 않은 시각에 에어컨이 켜질 수 있고(끄기와 위험 비대칭), 실사례의 원인은
   * 재전송으로 해결되지 않는 **설정 문제**(2in1 쓰기 인덱스 교차)였기 때문이다.
   * 그래서 조치 방법을 문구에 담아 **사용자가 고칠 수 있게** 한다.
   */
  _scheduleOnAbsorbVerify() {
    if (this._onVerifyTimer) clearTimeout(this._onVerifyTimer);
    this._onVerifyTimer = setTimeout(async () => {
      this._onVerifyTimer = null;
      if (this._stopped) return;
      if (this._offIntentTs !== 0) return;             // 그 사이 끄기 의도가 들어옴
      try {
        await this.getCachedState(true, 0, true);       // 반드시 새 실측
      } catch (e) {
        // ★폴은 상태만 갱신할 뿐 흡수 검증을 다시 하지 않는다 — 여기서 포기하면 영영 모른다.
        //   (끄기쪽이 v2.1.2에서 같은 논리로 이미 정정됐는데 켜기쪽에 전파하지 않았다)
        this._onVerifyRetry = (this._onVerifyRetry || 0) + 1;
        if (this._onVerifyRetry <= 2) { this._scheduleOnAbsorbVerify(); return; }
        this._onVerifyRetry = 0;
        this.log.warn(`[${this.name}] 켜기 반영을 확인하지 못했습니다(실측 3회 실패) — 기기가 켜졌는지 직접 확인해 주세요: ${e.message}`);
        return;
      }
      if (this._stopped || this._offIntentTs !== 0) return;
      if (this.deviceState?.Operation?.power === CONSTANTS.POWER.ON) {
        this._onAbsorbMiss = 0;   // ★정상 확인 시 리셋 — 안 하면 몇 달 간격의 무관한 미스가 합산돼 "N회"가 거짓이 된다
        return;
      }
      this._onAbsorbMiss = (this._onAbsorbMiss || 0) + 1;
      // 첫 1회는 타이밍(리모컨/앱과 겹침)일 수 있으니 조용히, 2회부터 알린다.
      if (this._onAbsorbMiss < 2) {
        this.debugLog('켜기 흡수 검증 — 기기가 아직 꺼져 있음(1회, 타이밍 가능)');
        return;
      }
      this.log.warn(`[${this.name}] 켜기가 반영되지 않았습니다 — 기기 상태가 계속 '꺼짐'으로 읽힙니다(${this._onAbsorbMiss}회). `
        + '2in1(같은 IP에 항목 2개)이라면 쓰기 인덱스가 읽기와 교차일 수 있습니다 — '
        + '두 항목의 쓰기 인덱스를 서로 바꿔 보세요. 홈 앱에서 엉뚱한 방이 켜지면 그 증상입니다.');
    }, 4000);
  }

  _scheduleOffAbsorbVerify(attempt = 1) {
    const VERIFY_DELAYS_MS = [2000, 5000, 15000];
    if (attempt > VERIFY_DELAYS_MS.length) return;
    ...
    this._offVerifyTimer = setTimeout(async () => {
      ...
      try {
        ...
        // v2.1.2 — 실측 실패를 삼키지 않는다. 옛 주석은 "다음 폴이 보정"이라 했으나 사실이 아니다:
        // _poll은 deviceState만 갱신할 뿐 OFF를 재전송하는 경로가 없어, 여기서 끝내면 오흡수가
        // 영구히 남는다(기기가 켜진 채 밤을 남). 단 _scheduleOffRetry로 넘기면 상태 확인 없이
        // OFF를 쏘므로 진짜 꺼진 기기에 '띠'가 울린다 → **재전송이 아니라 실측을 재시도**한다.
        // OFF는 실측이 '켜짐'을 확인한 뒤에만 나가므로 무음 보장은 그대로 유지된다.
        if (attempt < VERIFY_DELAYS_MS.length) {
          this.debugLog(`끄기 흡수 검증 실측 실패(${e.message}) — ${VERIFY_DELAYS_MS[attempt] / 1000}초 뒤 재시도 (${attempt}/${VERIFY_DELAYS_MS.length - 1}).`);
          if (this._offIntentTs === intentTs) this._scheduleOffAbsorbVerify(attempt + 1);
        } else {
          this.log.warn(`[${this.name}] 끄기 흡수 검증 실측 ${VERIFY_DELAYS_MS.length}회 모두 실패 — 기기가 켜진 채 남았을 수 있습니다: ${e.message}`);
        }
        return;
      }
  ...
  _schedulePowerOnResends() {
    ...
    // 아래 3개는 테스트가 대기를 축약할 수 있도록 인스턴스 필드 오버라이드를 허용한다
    // (_onGuardMs를 축약하는 기존 하네스와 같은 방식). 평시에는 상수를 쓴다.
    const confirmMaxTries = Number.isFinite(this._powerOnConfirmMaxTries)
      ? this._powerOnConfirmMaxTries : CONSTANTS.POWER_ON_CONFIRM_MAX_TRIES;
    const confirmGapMs = Number.isFinite(this._powerOnConfirmGapMs)
      ? this._powerOnConfirmGapMs : CONSTANTS.POWER_ON_CONFIRM_GAP_MS;
    const confirmBudgetMs = Number.isFinite(this._powerOnConfirmBudgetMs)
      ? this._powerOnConfirmBudgetMs : CONSTANTS.POWER_ON_CONFIRM_BUDGET_MS;
    if (this._powerOnModeTimer) clearTimeout(this._powerOnModeTimer);
    this._powerOnResendGen += 1;
    const gen = this._powerOnResendGen;
    this._resendAbortedAt = 0;   // 새 켜기가 시작되면 옛 포기 시각의 시계는 버린다
    let idx = 0;
    // v2.14.9 — 확인 재시도 계측. 중단했을 때 "기기가 늦게 켜진 것"과 "조회 자체가 실패한 것"을
    // 로그만 보고 갈라야 한다. 이전에는 둘 다 '기기 전원 Off 확인' 한 줄로 같아 보였고,
    // 조회 실패는 .catch로 삼켜 옛 값이 남을 뿐이라 구분할 근거가 아예 없었다(2026-08-11 침실).
    // 단계마다 새로 센다 — 상한은 단계별이지 체인 전체가 아니다(개편 전 fire(0) 리셋과 동치).
    let confirm = null;
    const fire = () => {
      this._powerOnModeTimer = null;
      if (this._stopped || gen !== this._powerOnResendGen) return;
      // Active/TargetState 중복 power-on 레이스로 윈도우가 재개장됐으면 남은 시간만큼 재대기.
      // 보호창 대기는 확인 예산에 넣지 않는다(기기 응답을 기다린 시간이 아니다) — v2.14.10:
      // confirm 이 이미 만들어진 뒤 재개장되면 그 대기가 예산을 잠식하므로 startedAt 을 밀어 준다.
      // (현재 호출부는 재개장 직후 체인을 재예약해 gen 으로 끊기지만, 리팩터 방어로 남긴다.)
      if (this._isOnGuardActive()) {
        const rewaitMs = Math.max(this._onGuardUntil - Date.now(), 0) + 100;
        if (confirm) confirm.startedAt += rewaitMs;
        this._powerOnModeTimer = setTimeout(fire, rewaitMs);
        return;
      }
      if (this.deviceState?.Operation?.power !== CONSTANTS.POWER.ON) {
        ...
        // 강제 조회 → 간격 대기 → 재판정을 반복해 느린 firmware에 반영 여유를 준다.
        // (v1.8.22: 2회→4회 / v2.14.9: 4회(≈9초)로도 부족한 실측이 나와 16회로 늘리고
        //  총 대기 상한을 함께 뒀다. HomeKit발 OFF는 타이머/세대로 즉시 끊기므로 오래 머물러도 안전.)
        if (!confirm) confirm = { tries: 0, ok: 0, fail: 0, startedAt: Date.now() };
        const waitedMs = Date.now() - confirm.startedAt;
        if (confirm.tries < confirmMaxTries && waitedMs < confirmBudgetMs) {
          confirm.tries += 1;
          // ★v2.14.10 — forceFresh(3번째 인자)가 필수다. 이것 없이 `getCachedState(true)` 만
          // 쓰면 **진행 중인 폴링 조회에 합류**하고(합류 지점은 getCachedState 의 stateRequestPromise
          // 분기), 그 폴링은 클라이언트의 공유 창(POLL_SHARE_MAX_AGE_MS=7초) 응답일 수 있다.
          // 그러면 "명령 이전 스냅샷"을 받고도 성공으로 세어져, 로그의 `조회 성공 N` 이
          // "기기를 N번 읽었는데 Off" 라는 뜻이 아니게 된다 — 진단이 통째로 거짓이 된다.
          this.getCachedState(true, 0, true).then(
            () => { confirm.ok += 1; },
            () => { confirm.fail += 1; },
          ).then(() => {
            if (this._stopped || gen !== this._powerOnResendGen) return;
            this._powerOnModeTimer = setTimeout(fire, confirmGapMs);
          });
          return;
        }
        // 상한까지 확인해도 Off → 진짜 꺼짐. 침묵 취소가 진단을 어렵게 했으므로 로그를 남긴다.
        // 조회 성공/실패를 나눠 적는 이유: 성공인데 Off면 기기가 늦은 것이고, 실패면 우리가
        // 못 읽은 것이라 조치가 정반대다.
        const cause = waitedMs >= confirmBudgetMs ? '대기 시간 초과' : '확인 한도 도달';
        const lastPower = this.deviceState?.Operation?.power ?? '값 없음';
        this.log.info(`[${this.name}] 전원 ON 후속 재전송 중단 — 기기 전원 Off 확인 `
          + `(${cause}: ${confirm.tries}회 ${Math.round(waitedMs / 1000)}초 · 상태 조회 성공 ${confirm.ok}·실패 ${confirm.fail}`
          + ` · 마지막 전원값 ${lastPower}) (남은 단계: ${steps.slice(idx).map(s => s.label).join(', ')})`);
        // v2.14.10 — 예산을 몇 초로 잡아야 하는지 정하려면 "포기한 뒤 몇 초 만에 켜짐이
        // 보였나"가 필요한데 아무도 그 값을 기록하지 않았다(8/11 사고 때도 HA 히스토리로
        // 소급 측정해야 했다). 다음 폴링이 On 을 처음 보면 그 간격을 남긴다.
        this._resendAbortedAt = Date.now();
        return;
      }
      confirm = null;   // 켜짐 확인 — 다음 단계는 새 예산으로 센다
      ...
      this.sendCommand(step.endpoint || '/mode', step.data)
        .then(() => {
          ...
          idx += 1;
          if (idx < steps.length) {
            this._powerOnModeTimer = setTimeout(fire, stepGapMs);
          } else {
            this._scheduleRefresh();
          }
        })
    ...
    this._powerOnModeTimer = setTimeout(fire, delay);
  }
  ...
  async _sendUnlessOnGuard(endpoint, data, patchFn) {
    ...
    if (this._isOnGuardActive() || (this._onGuardMs > 0 && this._activeInFlight === CONSTANTS.POWER.ON)) {
      ...
      if (this._onGuardStrategy === 'drop') {
        this.log.info(`[${this.name}] ON 보호 중 명령 무시: ${endpoint || '전원'} → ${JSON.stringify(data)}`);
      } else {
        this.debugLog(`ON 보호 큐에 추가: ${endpoint || '전원'} → ${JSON.stringify(data)}`);
      }
      return;
    }
  ...
  startPolling() {
    ...
    if (Number.isFinite(interval) && interval >= 1) {
      ...
      this.log.info(`[${this.name}] 상태 폴링 시작 (${this.pollingInterval}s)`);
      this._poll();
    }
  }
  ...
  async _poll() {
    ...
    try {
      ...
      // v2.0.0 — 문턱 >1 → >0: 실패 1회도 error가 찍히므로(626행) 복구도 짝 맞춰 알린다.
      if (this._pollFailStreak > 0) this.log.info(`[${this.name}] 폴링 복구 — 연속 ${this._pollFailStreak}회 실패 후 정상화`);
      // ★재시작 구멍(2026-08-05 적대 리뷰): 실패 스트릭은 **메모리에만** 있다. 🔴가 걸린 뒤
      //   홈브릿지가 재시작되면 streak 이 0 으로 시작해 복구 문구가 영영 안 나오고, 감시기의
      //   경보는 6시간마다 계속 울린다. 부팅 후 **첫 폴링이 실제로 성공했을 때** 한 번만
      //   접촉선을 남겨 짝을 맞춘다(가짜 복구가 아니다 — 정말로 기기에 닿았을 때만 찍힌다).
      if (!this._contactAnnounced) {
        this._contactAnnounced = true;
        this.log.info(`[${this.name}] 기기 접속됨 — 상태 조회 정상`);
      }
      this._pollFailStreak = 0;
    }
    catch (e) {
  ...
  // v2.14.10 — 후속 체인이 "전원 Off"로 포기한 뒤 기기가 실제로 켜짐을 보고하기까지 얼마나
  // 더 걸렸는지 한 번만 남긴다. 확인 예산을 몇 초로 잡아야 하는지 정할 유일한 수치인데,
  // 2026-08-11 사고 때는 아무도 기록하지 않아 HA 히스토리로 소급 측정해야 했다.
  _noteLatePowerOn() {
    if (!this._resendAbortedAt) return;
    if (this.deviceState?.Operation?.power !== CONSTANTS.POWER.ON) return;
    const lateSec = Math.round((Date.now() - this._resendAbortedAt) / 1000);
    this._resendAbortedAt = 0;
    this.log.info(`[${this.name}] 후속 재전송을 포기한 뒤 ${lateSec}초 만에 기기가 켜짐을 보고했습니다`
      + ` — 확인 대기가 그만큼 모자랐다는 뜻입니다.`);
  }
  ...
  async getCachedState(force = false, maxAgeMs = 0, forceFresh = false) {
    ...
    const fetchPromise = (async () => {
      try {
        ...
        this._noteLatePowerOn();
        this._warnUnsupportedMode();
        // 조회가 성공하면 오류 시그니처를 지운다 — 다음 장애는 다시 error로 눈에 띄어야 한다.
        this._stateErrSig = null;
        ...
        if (/시간 초과|ETIMEDOUT|ECONNRESET|ECONNREFUSED|EHOSTUNREACH|ENOTFOUND|EPIPE|TLS 소켓 오류/.test(e.message)) {
          ...
          const sig = `${e.message}`;
          if (this._stateErrSig !== sig) {
            this._stateErrSig = sig;
            this.log.error(`[${this.name}] 상태 조회 오류: ${e.message} — 같은 오류는 반복 기록하지 않습니다.`);
          } else {
            this.log.debug(`[${this.name}] 상태 조회 오류(반복): ${e.message}`);
          }
        }
        throw e;
      }
  ...
  async sendCommand(endpoint, data) {
    ...
    const job = async () => {
      try {
        this.log.info(`[${this.name}] 전송 → ${endpoint || '전원'} ${JSON.stringify(data)}`);
  ...
  setupCharacteristics() {
    ...
    s.getCharacteristic(C.Active)
      ...
      .onSet(async (value) => {
        this.log.info(`[${this.name}] 전원 → ${value ? '켜짐' : '꺼짐'}`);
        ...
        if (currentPower === targetPower && this._activeInFlight !== opposite) {
          ...
          // ★v2.6.6 — 켜기 흡수도 검증한다(그동안 끄기만 검증하는 **비대칭**이었다).
          //   다른 사용자 실사례(2026-07-30): 2in1 두 항목이 같은 IP를 쓰는데 쓰기 인덱스가
          //   교차 배선인 기기에서, 명령이 옆 방으로 나가 **자기 상태가 안 바뀌고** →
          //   다음 반대 방향 탭이 "이미 그 상태"로 조용히 생략됐다. 로그엔 `전원 → 켜짐`만
          //   찍히고 전송은 없어(생략 안내가 debug라 debug:false면 아예 안 보임) 사용자는
          //   "제어가 안 된다"만 알 수 있었다. 검증해서 어긋나면 **사용자에게 알린다.**
          if (value === 1) this._scheduleOnAbsorbVerify();
          return;
        }
        ...
        try {
          ...
          this.log.error(`[${this.name}] 전원 설정 실패: ${e.message}`);
    ...
    s.getCharacteristic(C.TargetHeaterCoolerState)
      ...
      .onSet(async (value) => {
        ...
        try {
          ...
          this.log.error(`[${this.name}] 모드 설정 실패: ${e.message}`);
          throw e;
        }
      });
    ...
    const flushTempSet = async () => {
      ...
      try {
        ...
        this.log.error(`[${this.name}] 설정온도 변경 실패: ${e.message}`);
      }
    };
    s.getCharacteristic(C.CoolingThresholdTemperature)
      ...
      .onSet(async (value) => {
        this.log.info(`[${this.name}] 설정온도 → ${value}°C`);
    ...
    if (this.swingBinding !== 'none' && !this._hideSwingToggle) {
      s.getCharacteristic(C.SwingMode)
        ...
        .onSet(async (value) => {
          this.log.info(`[${this.name}] 스윙 → ${value ? '켜짐' : '꺼짐'}`);
          ...
          try {
            ...
            this._scheduleRefresh();
          } catch (e) { this.log.error(`[${this.name}] 바람 설정 실패: ${e.message}`); throw e; }
        });
    ...
    if (this.lockBinding !== 'none' && !this._hideLockToggle) {
      s.getCharacteristic(C.LockPhysicalControls)
        ...
        .onSet(async (value) => {
          this.log.info(`[${this.name}] 잠금 → ${value ? '켜짐' : '꺼짐'}`);
          ...
          try {
            ...
            this._scheduleRefresh();
          } catch (e) { this.log.error(`[${this.name}] 잠금 설정 실패: ${e.message}`); throw e; }
        });
