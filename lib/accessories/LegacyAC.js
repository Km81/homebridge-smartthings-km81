...
class LegacyAC {
  constructor({ log, config, api, accessory, packageRoot }) {
    ...
    // Command serialization mutex: ensures Active/TargetState setters don't
    // fire commands in parallel, which overwhelms the legacy AC and can
    // result in the unit turning off mid-sequence.
    this._cmdMutex = Promise.resolve();
    // Track when TargetState was last set so we can skip a redundant
    // powerOnMode application when HomeKit fires Active+TargetState together.
    ...
    this.log.info(`[${this.name}] LegacyAC 초기화 완료. (HK presets: ${
      Object.entries(this.hkPresets)
        .filter(([, p]) => p.enabled)
        .map(([n, p]) => `${n}=[${p.modes.join(',')}]${p.options.length ? `+opt(${p.options.join(',')})` : ''}`)
        .join(' / ') || '(none)'
    })`);
  }
  ...
  _buildHkPresets() {
    ...
    const resolve = (modeField, wfField, enabledField, modesField, optionsField, defaults) => {
      const mode = cfg[modeField];
      if (typeof mode === 'string' && mode.length > 0) {
        if (mode === 'none') return { enabled: false, modes: [], options: [] };
        const opts = cfg[wfField] === true ? ['Comode_Nano'] : [];
        return { enabled: true, modes: [mode], options: opts };
      }
      ...
      return {
        ...
        options: norm(cfg[optionsField])
      };
    };

    return {
      cool: resolve('hkCoolMode', 'hkCoolWithWindFree', 'hkCoolEnabled', 'hkCoolModes', 'hkCoolOptions',
        { enabled: true, modes: ['Cool'] }),
      heat: resolve('hkHeatMode', 'hkHeatWithWindFree', 'hkHeatEnabled', 'hkHeatModes', 'hkHeatOptions',
        { enabled: false, modes: [] }),
      auto: resolve('hkAutoMode', 'hkAutoWithWindFree', 'hkAutoEnabled', 'hkAutoModes', 'hkAutoOptions',
        { enabled: false, modes: [] }),
    };
  }
  ...
  _resolvePowerOnPreset() {
    ...
    if (typeof cfg.powerOnMode === 'string' && cfg.powerOnMode.length > 0) {
      ...
      return {
        label: cfg.powerOnMode + (cfg.powerOnWithWindFree ? ' + WindFree' : ''),
        modes: [cfg.powerOnMode],
        options: cfg.powerOnWithWindFree === true ? ['Comode_Nano'] : []
      };
    }
  ...
  async sendCommand(endpoint, data) {
    // Serialize all commands so multiple HomeKit setters (Active + TargetState
    // + SwingMode firing in parallel) don't interleave at the network layer.
    const job = async () => {
  ...
  setupCharacteristics() {
    ...
    s.getCharacteristic(C.Active)
      ...
      .onSet(async (value) => {
        ...
        try {
          ...
          if (value === 1) {
            // HomeKit often fires Active=1 together with a TargetState change
            // (e.g., tile tap + mode select). If TargetState was just set, its
            // setter already pushed the correct mode -- applying powerOnMode
            // here would just overwrite it. Skip in that case.
    ...
    // ===== TargetHeaterCoolerState (with configurable HK->AC mapping) =====
    ...
    // ===== SwingMode =====
    // HomeKit's tile UI sometimes auto-restores SwingMode when the user taps
    // the tile to turn the AC on, which can cause unexpected swing/WindFree
    // toggles. Set enableSwingMode=false in config to hide it entirely.
    ...
    // ===== LockPhysicalControls (autoClean toggle) =====
