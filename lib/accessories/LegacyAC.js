...
class LegacyAC {
  ...
  _buildHkPresets() {
    ...
    // v1.2.0+ 단순 형태: hkCoolMode (enum) + hkCoolWithWindFree (bool)
    // 기존 v1.1.x 형태: hkCoolEnabled + hkCoolModes[] + hkCoolOptions[] (fallback)
    const resolve = (modeField, wfField, enabledField, modesField, optionsField, defaults) => {
      ...
      // fallback to legacy v1.1.x fields
  ...
  _resolvePowerOnPreset() {
    const cfg = this.config;
    // v1.2.0+ 직접 모드 지정: powerOnMode (enum) + powerOnWithWindFree (bool)
    ...
    // fallback to legacy powerOnHkMode (refers to one of cool/heat/auto presets)
  ...
  setupCharacteristics() {
    ...
    // ===== SwingMode (comfort = WindFree toggle / wind = up-down swing) =====
