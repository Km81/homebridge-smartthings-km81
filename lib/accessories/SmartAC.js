...
class SmartAC {
  ...
  // v1.8.18 — 전원 ON 후속 모드 재전송 (opt-in, resendModeOnPowerOn).
  // HomeKit이 타일 탭/Siri에서는 Active만 보내 모드 명령이 아예 없고, TargetState를 함께 보내는
  // 경우에도 기기가 켜지는 중이라 모드를 놓칠 수 있다. ON 후 일정 지연 뒤 설정 모드
  // (coolModeCommand, 예: dryClean)를 1회 전송해 항상 설정 모드로 켜지도록 보장한다.
  _schedulePowerOnModeResend(deviceId, coolCmd, displayName) {
    if (this._powerOnModeTimer) clearTimeout(this._powerOnModeTimer);
    const fire = (retried) => {
      this._powerOnModeTimer = null;
      if (this._stopped) return;
      if (this._state.power !== true) {
        ...
        // HomeKit발 OFF는 setter가 타이머를 직접 지우므로 여기 도달하지 않는다.
        if (!retried) {
      ...
      this.log.info(`[${displayName}] 전원 ON 후속 모드 재전송: ${coolCmd}`);
      this.smartthings.setMode(deviceId, coolCmd)
        .catch(e => this.log.warn(`[${displayName}] 전원 ON 후속 모드 재전송 실패: ${e.message}`));
    };
  ...
  _setupHeaterCooler(accessory, configDevice) {
    ...
    this._bindCharacteristic({
      ...
      setter: async (value) => {
        ...
        if (!target && this._powerOnModeTimer) {
          clearTimeout(this._powerOnModeTimer);
          this._powerOnModeTimer = null;
        }
        await this.smartthings.setPower(deviceId, target);
        this._state.power = target;
        if (target && resendModeOnPowerOn) {
          this._schedulePowerOnModeResend(deviceId, coolCmd, service.displayName);
        }
