...
const MIN_BACKGROUND_POLL_SEC = 30;
...
class SmartAC {
  ...
  // seq tag로 빠른 연속 토글 시 stale 결과가 새 _state를 덮어쓰지 않도록 보호.
  ...
  _setupHeaterCooler(accessory, configDevice) {
    ...
    this._bindCharacteristic({
      ...
      setter: async (value) => {
        ...
        // Idempotency: 이미 같은 모드 명령을 직전에 보냈으면 생략 (HomeKit 재assertion 흡수)
        if (this._lastSentMode === coolCmd) {
          this.log.debug?.(`[${service.displayName}] Mode 이미 ${coolCmd} — 명령 생략`);
          return;
        }
        await this.smartthings.setMode(deviceId, coolCmd);
        this._lastSentMode = coolCmd;
      },
    });
  ...
  _setupBackgroundPolling(accessory, configDevice) {
    const sec = Number(configDevice.pollingInterval);
    if (!Number.isFinite(sec) || sec < MIN_BACKGROUND_POLL_SEC) return;
    const C = this.Characteristic;
    const deviceId = accessory.context.device.deviceId;
