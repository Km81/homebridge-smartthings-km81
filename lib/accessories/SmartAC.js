...
// v1.8.12: 최소 폴링 간격을 5초까지 허용 (사용자 체감 개선).
// 5초도 resync(2s) + fetch(~0.5s) = 2.5s 점유 + 2.5s 여유로 race-safe하다는 점이 시뮬레이션으로 확인됨.
// SmartThings API rate limit(250 req/min/token)에도 충분히 여유 (1 SmartAC × 12회/min = 4.8%).
// 5초 미만은 token burst limit / resync race 위험이 빠르게 커진다.
const MIN_BACKGROUND_POLL_SEC = 5;
