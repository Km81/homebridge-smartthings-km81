...
class SmartAC {
  ...
  configure(accessory, configDevice, packageVersion) {
    ...
    // ★★2026-08-03 — **지난 부팅의 마지막 값으로 시작한다.**
    //   콜드 부팅에는 `_state`가 비어 있어 홈킷 read 6발이 전부 기기까지 가는데, 로컬 경로는
    //   기기당 DTLS 핸드셰이크를 **순차로** 하느라 두 번째 기기는 6초가 걸린다. 그동안
    //   6개 특성이 stall하며 홈브릿지 `slow to respond`(임계 3초)가 재시작마다 6줄씩 났다.
    //   ⚠️**옛 값을 보여주는 대가**가 있다 — 홈브릿지가 꺼져 있는 동안 리모컨으로 조작했다면
    //     최대 폴링 주기(10초)만큼 어긋난 값이 보인다. 곧바로 폴이 정정한다.
    //   ★그래도 이쪽이 낫다: 홈킷은 원래 자기 캐시의 마지막 값을 들고 있고, 여기서 6초간
    //     stall하면 그 사이 홈 앱은 **'응답 없음'**을 띄운다. 옛 값 10초 < 응답 없음 6초.
    //   ⚠️`_seeded`에는 넣지 않는다 — 시드는 **추정**이고, 기기가 실제로 그 값을 보고한
    //     적은 없다. 폴이 실패하면 다음 read가 다시 기기에 물어야 한다.
    //   ⚠️★**폴링을 끈 구성에서는 시드를 쓰지 않는다**(적대 리뷰 A-M1 / B-LOW-2).
    //     `_lazyGet`은 `_state[k]`가 정의돼 있으면 즉답하므로, 갱신해 줄 폴이 없으면
    //     지난 부팅 값이 **영원히** 표시된다. 수정 전에는 콜드 리드가 기기까지 가서
    //     죽은 기기면 정직하게 '응답 없음'이 났다 — 그 정직함을 시드로 없애면 안 된다.
    //     (`_seeded`에 안 넣는 것만으로는 못 막는다. `_state`가 정의되는 순간 read가 멈춘다.)
    const pollRaw = configDevice.pollingInterval;
    const pollingOn = !(pollRaw != null && pollRaw !== '' && !(Number(pollRaw) > 0));
    this._stateFresh = false;                       // 시드값은 신선하지 않다(켜기 생략 금지)
    const remembered = pollingOn ? accessory.context?.km81LastState : null;
    if (remembered && typeof remembered === 'object') {
      let n = 0;
      for (const k of Object.keys(this._state)) {
        if (remembered[k] !== undefined && remembered[k] !== null) { this._state[k] = remembered[k]; n += 1; }
      }
      if (n > 0) this.log.debug?.(`[${accessory.displayName}] 지난 부팅의 값 ${n}개로 시작 (폴이 곧 갱신)`);
    }
    this._accessory = accessory;
    this._lastPersistTs = 0;
    this._firstPollDone = false;   // 첫 성공 폴은 값이 같아도 특성을 밀어 준다(재조정)
    this._writeGen = 0;            // 사용자 명령 세대 — 폴이 낡은 값으로 덮는 것을 막는다
    ...
    // ★이 기기가 만든 별도 액세서리(옵션 스위치) — 임시 연결해제 봉인 대상에 함께 넣는다.
    this._optionalSwitchAccessories = [];
    ...
    // ★「임시 연결해제」 — 서비스를 다 만든 뒤에 **폴링만** 안 건다(방 배치·장면은 그대로).
    //   ⚠️registerShutdown 은 아래에서 그대로 등록한다 — 여기서 return 하면 종료 훅이 사라진다.
    if (isPutAway(configDevice)) {
      this._stopped = true;        // ★이중 잠금 — 명령 뒤 재확인(_scheduleResync)도 시작하지 않는다
      // ★마지막 상태를 정상 연결로 표시(putAway.js 주석 참조). ⚠️본체만 봉하면 안 된다 —
      //   무풍·자동건조 스위치는 **별도 액세서리**라 그대로 두면 탭이 명령을 보낸다(2026-09-15 리뷰).
      const sealed = sealForPutAway([accessory, ...this._optionalSwitchAccessories]);
      this.log.info(`[${accessory.displayName}] ${PUT_AWAY_MESSAGE}`);
      if (sealed === 0) this.log.warn(`[${accessory.displayName}] ${PUT_AWAY_SEAL_EMPTY}`);
    } else {
    ...
    this.platform.registerShutdown(() => {
      ...
      if (this._powerOnModeTimer) clearTimeout(this._powerOnModeTimer);
      if (this._offRetryTimer) clearTimeout(this._offRetryTimer);
      // ★종료 직전에 한 번은 반드시 남긴다 — 저장 간격(5분) 때문에 마지막 변화를
      //   놓치면, 재시작 직후 시드가 그만큼 더 낡는다.
      this._lastPersistTs = 0;
      this._rememberState();
    });
  }
  ...
  _schedulePowerOnResends(deviceId, { mode, autoClean, stepMs, displayName }) {
    ...
    const fire = (retried) => {
      ...
      if (this._state.power !== true) {
        ...
        // v2.14.9 — 재확인에도 off → 진짜 꺼짐, 남은 체인 전체 취소.
        // 조용한 취소는 "왜 자동건조가 안 켜졌나"를 로그로 추적할 수 없게 만든다. LegacyAC는
        // v1.8.22부터 중단 로그를 남기는데 여기에는 전파되지 않아 한쪽만 진단 불가였다.
        // ⚠️v2.14.10 — 회수를 하드코딩("1회")했더니 재확인 경로가 사라져도 로그는 그대로
        //   "1회"라고 말했다(적대 리뷰가 변이로 실증). 실제 값에서 뽑는다.
        this.log.info(`[${displayName}] 전원 ON 후속 재전송 중단 — 기기 전원 Off 확인 `
          + `(재확인 ${retried ? 1 : 0}회 · 남은 단계: ${steps.slice(idx).map(s => s.label).join(', ')})`);
        return;
      }
  ...
  /**
   * 지금 상태를 액세서리 컨텍스트에 적어 둔다 — 다음 부팅의 콜드 리드가 이걸로 즉답한다.
   * ⚠️디스크 쓰기는 홈브릿지가 하므로 자주 부르면 안 된다. 값이 **실제로 바뀐 때만**,
   *   그것도 최소 간격을 두고 저장한다(에어컨 온도는 1분에도 여러 번 흔들린다).
   */
  _rememberState() {
    const acc = this._accessory;
    if (!acc || !acc.context) return;
    const prev = acc.context.km81LastState;
    const next = {};
    let changed = false;
    for (const [k, v] of Object.entries(this._state)) {
      if (v === undefined) { if (prev && prev[k] !== undefined) next[k] = prev[k]; continue; }
      next[k] = v;
      if (!prev || prev[k] !== v) changed = true;
    }
    if (changed) {
      acc.context.km81LastState = next;
      // ⚠️"기억을 갱신했다"와 "디스크에 반영했다"는 다르다. 둘을 한 조건으로 묶으면,
      //   간격에 막혀 미룬 변화가 **영영 저장되지 않는다**(다음 호출은 changed=false).
      this._persistDirty = true;
    }
    if (!this._persistDirty) return;
    // 홈브릿지에 "이 액세서리 컨텍스트가 바뀌었다"고 알린다(디스크 반영).
    const now = Date.now();
    if (now - this._lastPersistTs < SmartAC.PERSIST_MIN_MS) return;
    this._lastPersistTs = now;
    this._persistDirty = false;
    try { this.api.updatePlatformAccessories([acc]); }
    catch (e) { this.log.debug?.(`[${this._label}] 상태 기억 실패(무해): ${e.message}`); }
  }
  ...
  _bindCharacteristic({ service, characteristic, props, getter, setter }) {
    ...
    if (setter) {
      char.on('set', async (value, callback) => {
        // ★★2026-08-03 — 쓰기 세대(적대 리뷰 M3). 폴 라운드가 값을 다 읽은 **뒤에** 사용자
        //   명령이 완료되면, 라운드 마무리가 낡은 값을 특성에 밀어 넣는다. 끄기 직후라면
        //   HA에 **2~10초짜리 유령 ON**이 뜨고, '에어컨 켜짐' 자동화가 에지에서 발화할 수 있다
        //   (8/2 유령 엔티티 사고가 이 민감성을 보여줬다).
        //   여기 한 곳에서 세대를 올린다 — 홈킷도 MQTT도 결국 이 set 핸들러를 지난다.
  ...
  async _setWindDirection(deviceId, target, originService) {
    ...
    // ★★2026-08-03 — **폴이 실패 중이면 생략하지 않는다**(적대 리뷰 M1). v2.11.0의 M6을
    //   전원에만 적용하고 여기 셋(무풍·자동건조·바람방향)에는 전파하지 않았다.
    //   생략의 전제는 `_state`가 기기의 현재 상태와 같다는 것인데, 그 값을 채우는 폴이
    //   죽으면 얼어붙는다. 그 상태의 탭은 **전송 0회인데 홈킷·HA에는 성공으로 보고**된다.
    //   ⚠️시드로 시작한 부팅 직후(첫 폴 전)도 같은 창이다.
    if (this._state.windDirection === target && this._stateFresh !== false) {
  ...
  async _setWindFree(deviceId, target, originService) {
    ...
    //   죽으면 얼어붙는다. 그 상태의 탭은 **전송 0회인데 홈킷·HA에는 성공으로 보고**된다.
    //   ⚠️시드로 시작한 부팅 직후(첫 폴 전)도 같은 창이다.
    if (this._state.windFree === target && this._stateFresh !== false) {
  ...
  async _setAutoClean(deviceId, target, originService) {
    ...
    //   죽으면 얼어붙는다. 그 상태의 탭은 **전송 0회인데 홈킷·HA에는 성공으로 보고**된다.
    //   ⚠️시드로 시작한 부팅 직후(첫 폴 전)도 같은 창이다.
    if (this._state.autoClean === target && this._stateFresh !== false) {
  ...
  _setupOptionalSwitches(device, configDevice, packageVersion) {
    ...
    const maybeCreateSwitch = (keySuffix, displayName, stateKey, setterFn, fetchFn) => {
      ...
      this._optionalSwitchAccessories.push(acc);
    ...
    const pollOnce = async () => {
      ...
      try {
        ...
        if (this._resyncTimers.size === 0) {
          // ★라운드 시작 시점의 쓰기 세대. 조회하는 동안 사용자 명령이 끼어들었는지 본다.
          ...
          // ★★2026-08-03 — 조회하는 동안 사용자 명령이 끼어들었으면 **이번 라운드를 버린다**
          //   (적대 리뷰 M3). 지금 읽은 값은 그 명령보다 **앞선** 상태다 — 밀어 넣으면 방금
          //   한 조작이 화면에서 되돌아간다. 끄기 직후라면 HA에 유령 ON이 뜨고
          //   '에어컨 켜짐' 자동화가 에지에서 발화할 수 있다.
          //   다음 폴(또는 resync)이 곧 정확한 값을 가져온다.
          if (this._writeGen !== genAtStart) {
            this.log.debug?.(`[${accessory.displayName}] 폴 도중 명령이 들어와 이번 라운드 결과를 버립니다`);
            return;
          }

          // ★★2026-08-03 — **첫 성공 폴은 값이 같아도 전 특성을 한 번 밀어 준다**(적대 리뷰 M2).
          //   v2.11.2까지는 `_state`가 전부 undefined로 시작해 첫 폴이 **무조건** push했고,
          //   그것이 홈킷 특성과 기기 상태가 어긋났을 때의 **암묵적 자가치유**였다
          //   (예: 끄기 장면의 형제 write를 억제했는데 되돌리는 resync가 순단으로 실패한 경우 —
          //    특성엔 장면값이 커밋돼 있고 `_state`엔 기기값이 남는다).
          //   v2.11.3 시드가 `_state`를 미리 채우면서 그 재조정이 통째로 사라졌다 →
          //   어긋난 표시가 **재시작을 넘어 영구 고착**된다. 부팅당 push 6회로 되돌린다.
          const forcePush = !this._firstPollDone;
          this._firstPollDone = true;

          if (forcePush || this._state.power !== power) {
          ...
          if (forcePush || this._state.windFree !== windFree) {
          ...
          if (forcePush || this._state.autoClean !== autoClean) {
          ...
          // ★기억은 **이번 라운드 값을 다 반영한 뒤에** 남긴다(적대 리뷰 L3).
          //   위쪽(갱신 전)에서 부르면 디스크 스냅샷이 **항상 한 라운드 낡는다** —
          //   비정상 종료 때 시드가 그만큼 더 어긋난다.
          this._rememberState();      // 다음 부팅이 이 값으로 즉답한다
        }
      } catch (e) {
...
// 상태를 디스크에 반영하는 최소 간격. 에어컨 실내온도는 1분에도 여러 번 흔들려서,
// 간격이 없으면 홈브릿지 cachedAccessories 쓰기가 폴링마다 일어난다.
SmartAC.PERSIST_MIN_MS = 5 * 60 * 1000;
