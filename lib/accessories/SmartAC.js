...
class SmartAC {
  ...
  _setupHeaterCooler(accessory, configDevice) {
    ...
    if (SmartAC.SWING_DIRECTIONS.includes(swingBinding)) {
      ...
      this._bindCharacteristic({
        ...
        setter: async (value) => {
          ...
          if (target) await this._setWindDirection(deviceId, target, service);
        },
      });
    ...
    const pollOnce = async () => {
      ...
      try {
        ...
        if (this._resyncTimers.size === 0) {
          ...
          const windDir = SmartAC.SWING_DIRECTIONS.includes(this._swingBinding)
