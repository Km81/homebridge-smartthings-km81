...
const OFF_SCENE_SUPPRESS_MS = 4000;
// v1.8.26 — 끄기 전송 실패 시 1회 지연 재시도 간격(ms). off는 멱등이라 안전.
const OFF_RETRY_DELAY_MS = 5000;
...
class SmartAC {
  ...
  // v1.8.26 — 끄기 전송이 "실패"했을 때만 1회 지연 재시도(성공 경로 무변경).
  // 그 사이 ON 의도가 오면(_offIntentTs 해제/갱신) 포기한다.
  _scheduleOffRetry(deviceId, service) {
    if (this._offRetryTimer) clearTimeout(this._offRetryTimer);
    this.log.warn(`[${service.displayName}] 끄기 전송 실패 — ${OFF_RETRY_DELAY_MS / 1000}초 뒤 1회 재시도합니다.`);
