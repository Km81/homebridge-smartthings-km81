...
class SmartAC {
  ...
  configure(accessory, configDevice, packageVersion) {
    ...
    // 모든 키는 lazy로 채워지며 (값이 undefined일 때만 실제 fetch),
    // 명령 송신 후 RESYNC_DELAY_MS 뒤 background refresh가 실제 값으로 보정한다.
    this._state = {
      power: undefined,
  ...
  // 같은 키로 빠른 연속 호출이 와도 마지막 호출 기준으로만 한 번 실행되도록 timer를 통합.
