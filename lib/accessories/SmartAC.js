...
class SmartAC {
  ...
  _setupHeaterCooler(accessory, configDevice) {
    ...
    this._swingWindDirection = String(configDevice.swingWindDirection || 'Up_And_Low');
    if (swingBinding === 'windDirection') {
      // 구형 에어컨은 스윙 토글을 무풍/상하 바람 중에서 고를 수 있는데 신형·시스템만
      // 무풍뿐이었다(v2.9.0). 켤 때 보낼 방향은 설정에서 고르고, 끄면 항상 고정(Fix).
      const onDir = String(configDevice.swingWindDirection || 'Up_And_Low');
      this._bindCharacteristic({
        service,
        ...
        setter: (value) => this._setWindDirection(deviceId, value === 1 ? onDir : 'Fix', service),
      });
  ...
  async _setWindDirection(deviceId, target, originService) {
    ...
    if (target !== 'Fix' && !this._warnedWindDir) {
      ...
      if (supported.length > 0 && !supported.includes(target)) {
        ...
        this.log.warn(`[${this._label}] 이 기기는 바람방향 '${target}'을 지원하지 않습니다 `
          + `— 지원: ${supported.join(', ')}. 설정의 '스윙 켤 때 보낼 바람방향'을 바꾸세요.`);
        return;
      }
    }
    ...
    const pollOnce = async () => {
      ...
      try {
        ...
        if (this._resyncTimers.size === 0) {
          ...
          const windDir = this._swingBinding === 'windDirection'
            ? await this.smartthings.getWindDirection(deviceId)
            : undefined;
