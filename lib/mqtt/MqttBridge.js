...
class MqttBridge {
   ...
   * ⚠️`'unknown'` 으로 떨어뜨리는 이유: HA 는 그 문자열을 **STATE_UNKNOWN** 으로 인식한다.
   *   빈 문자열은 "상태 갱신 건너뜀"이라 오히려 **옛 값이 남는다** — 우리가 막으려던 그것이다.
   */
  static _withTemplateDefault(tpl) {
    ...
    return `{{ ${m[1]} | default('unknown') }}`;
  }

  _publishDiscovery(component, slug, objectId, payload) {
    const topic = `${this.discoveryPrefix}/${component}/km81_${slug}/${objectId}/config`;
    const withDefault = (typeof payload.value_template === 'string')
      ? { ...payload, value_template: MqttBridge._withTemplateDefault(payload.value_template) }
      : payload;
  ...
  registerSmartAc({ slug, label, model, minTemp, maxTemp, tempStep, setChar,
    ...
    this._publishDiscovery('binary_sensor', slug, 'night_mode', {
      name: '열대야쾌면', unique_id: `km81_${slug}_night_mode`, device: dev, availability_topic: avail,
      state_topic: stateTopic, value_template: '{{ value_json.night_mode }}',
      payload_on: 'ON', payload_off: 'OFF', icon: 'mdi:weather-night',
    });
  ...
  // ── 세탁기·건조기 ────────────────────────────────────────────────────────
  registerLaundry({ slug, label, model, kind, hasEnergy, hasProgress, hasDetail }) {
  ...
  retractDevice({ slug, kind, label, model }) {
    ...
    try {
      ...
      if (kind === 'waterPurifier') this.registerWaterPurifier({ slug, label, model });
        ...
        this.registerLaundry({ slug, label, model, kind, hasEnergy: true, hasProgress: true, hasDetail: true });
  ...
  _retractDiscovery(component, slug, objectId) {
    this._publish(`${this.discoveryPrefix}/${component}/km81_${slug}/${objectId}/config`, '', { retain: true, qos: 1 });
  }
