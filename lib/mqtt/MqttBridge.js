...
class MqttBridge {
  ...
  registerSmartAc({ slug, label, model, minTemp, maxTemp, tempStep, setChar,
    ...
    mkSensor('night_elapsed_min', '열대야쾌면 경과', { unit_of_measurement: 'min', state_class: 'measurement', icon: 'mdi:sleep', entity_category: 'diagnostic' });
  ...
  publishSmartAcSensors(slug, s = {}) {
    ...
    if (s.night_elapsed_min !== undefined) {
      partial.night_elapsed_min = Number.isFinite(s.night_elapsed_min) ? s.night_elapsed_min : null;
    }
