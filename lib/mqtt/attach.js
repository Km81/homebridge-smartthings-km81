  ...
  if (local && deviceId) {
    ...
    if (disabled) {
      ...
      startPoller({
        ...
        run: async () => {
          const [filter, clean, alarm, self] = await Promise.all([
...
function attachLaundry({ bridge, api, log, accessory, logic, configDevice, slug, kind, platform }) {
  ...
  if (local && deviceId) {
    ...
    startPoller({
      ...
      run: async () => {
        const [remote, kids, setting, alarm] = await Promise.all([
...
function attachWaterPurifier({ bridge, log, client, deviceId, configDevice, slug, label, platform }) {
  ...
  startPoller({
    ...
    run: async () => {
      const [filter, status, setting, locks, self] = await Promise.all([
      ...
      const alive = !!(filter || status || setting || locks || self);
