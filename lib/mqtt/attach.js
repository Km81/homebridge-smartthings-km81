...
function attachLaundry({ bridge, api, log, accessory, logic, configDevice, slug, kind, platform }) {
  ...
  if (local && deviceId) {
    ...
    const pollLocal = async () => {
      // ⛔v2.14.17 — 여기에 합성 '꺼짐'을 넣었다가 되돌렸다(v2.14.16). 이유는 실측이다:
      //   폴 실패 시 **발행 자체가 없어야** retained 의 `last_seen` 이 보존되는데, 합성 꺼짐이
      //   발행을 만들면서 재기동 직후(`lastOk=0` → last_seen 생략) payload 가 retained 를 덮어
      //   HA 에서 `unknown` 이 됐다. 그러면 두절 센서가 `unknown` 을 두절로 안 보므로
      //   **죽은 기기가 '정상'으로 보인다** — 두절 감시가 통째로 무력화된다(2026-08-19 실측).
      //   ★홈킷 경로(Laundry.js)는 retained 개념이 없어 같은 문제가 없다. 그래서 그쪽만 적용한다.
      //   ⚠️여기에 다시 넣으려면 **MqttBridge 가 재기동 후에도 last_seen 을 보존**하도록
      //     먼저 고쳐야 한다(그 전에는 넣지 말 것).
