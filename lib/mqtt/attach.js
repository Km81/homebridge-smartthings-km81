...
const SEC = 1000;
const { classifyComponent, buildSyntheticOff } = require('../accessories/Laundry');
...
// `last_seen` 을 내보내도 되는 구성인가.
//
// ⛔**클라우드 폴백이 켜져 있으면 내보내지 않는다.** 두 방향으로 다 거짓말이 되기 때문이다:
//   ① 세탁물은 클라우드 조회 성공에도 훅이 발화한다 — 클라우드는 기기가 오프라인이어도
//      캐시 상태로 응답하므로 **죽은 기기의 last_seen 이 영원히 신선**해진다(거짓 안심).
//   ② 에어컨은 클라우드 폴백 성공이 `lastOk` 를 안 올린다 — 값은 갱신되는데 last_seen 만
//      늙어 **살아 있는 기기를 죽었다고** 보고한다.
//   둘 중 어느 쪽이든 HA 의 사망 판정을 망가뜨린다. 로컬 전용일 때만 의미가 성립한다.
//   (우리 집은 3대 전부 폴백 OFF라 그대로 나온다.)
//   ⚠️설정 위치는 `devices[].local.fallbackToCloud` 다 — 최상위가 아니다. 최상위로 읽으면
//     항상 undefined 라 **모든 기기에서 last_seen 이 조용히 사라진다**(계측기 schema_ui 가
//     "코드가 읽는 키가 스키마에 없다"로 잡아 줬다).
function lastSeenUsable(configDevice) {
  const local = configDevice && configDevice.local;
  return !!local && local.fallbackToCloud === false;
}
...
function lastSeenOf(client, deviceId) {
  ...
  try {
    ...
    // ⚠️0(=이 프로세스에서 아직 한 번도 성공 못 함)이면 **발행을 생략**한다(undefined).
    //   `null` 을 흘리면 키가 지워지고, retained 에 남아 있던 **마지막 수신 시각이 소멸**한다.
    //   기기가 죽은 채로 홈브릿지가 재시작되면 그 시각이 유일한 증거인데 그걸 우리가 지우게
    //   된다 — 이 기능이 잡으려는 바로 그 시나리오에서 실패한다(적대 리뷰 F2).
...
function attachSmartAc({ bridge, api, log, accessory, logic, configDevice, slug, platform }) {
  ...
  // ★폴링을 끈 기기(pollingInterval 0/음수)는 모니터링 폴러가 아예 안 돈다 — 그러면 센서도
  //   **채워질 수 없는 유령**이다. 판정을 등록 **앞으로** 옮겼다(적대 리뷰 F4: 예전엔 등록한
  //   뒤에 폴러만 막아서, 게이트가 있는데도 다른 문으로 유령이 들어왔다).
  const pollRaw = configDevice ? configDevice.pollingInterval : undefined;
  const pollN = Number(pollRaw);
  const pollDisabled = pollRaw != null && pollRaw !== '' && Number.isFinite(pollN) && pollN <= 0;
  const canMonitor = local && !!deviceId && !pollDisabled;
  ...
  bridge.registerSmartAc({
    ...
    hasMonitor: canMonitor,
    hasLastSeen: canMonitor && lastSeenUsable(configDevice),
  });
  emitCtl();

  // ── 모니터링 폴러(로컬 기기만) — 2026-08-04 부터 **두 티어** ──
  if (local && deviceId) {
    const disabled = pollDisabled;

    if (disabled) {
      log.debug?.(`[MQTT] '${accessory.displayName}' 폴링이 꺼져 있어 모니터링 센서를 갱신하지 않습니다.`);
    } else {
      const fastSec = Number.isFinite(pollN) && pollN >= 15 ? pollN : 60;
      // ⚠️설정값이 하한(15초)에 막혀 무시되면 **말해 준다** — 10을 적어 뒀는데 60으로 도는 걸
      //   모르면 "왜 안 따라오지?"로 헤맨다(2026-08-04).
      if (Number.isFinite(pollN) && pollN > 0 && pollN < 15) {
        log.info(`[MQTT] '${accessory.displayName}' 모니터링 주기는 최소 15초입니다 — `
          + `설정 ${pollN}초 대신 ${fastSec}초로 돕니다(홈킷 상태 폴링은 설정대로 ${pollN}초).`);
      }
...
function attachLaundry({ bridge, api, log, accessory, logic, configDevice, slug, kind, platform }) {
  ...
  bridge.registerLaundry({
    ...
    hasLastSeen: lastSeenUsable(configDevice),
  });
  ...
  if (local && deviceId) {
    ...
    let synthOffAnnounced = false;
    const pollLocal = async () => {
      let comps;
      try {
        comps = await client.getStatus(deviceId, { localOnly: true });   // 폴백 없음(MEDIUM-3)
        synthOffAnnounced = false;
      } catch (e) {
        // ★v2.14.16 — 홈킷 경로(Laundry)와 **같은 판정**으로 합성 '꺼짐'을 만든다.
        //   이게 없으면 기기가 죽어도 HA 는 옛 값에 고착된다 — 2026-08-19 실측으로
        //   홈킷은 `꺼짐`인데 HA 는 `운전 중 · 61% · 35분 남음` 이 그대로 남아 있었다
        //   (v2.14.15 가 홈킷만 고친 반쪽이었다). 판정은 buildSyntheticOff 하나를 공유한다.
        //   ⚠️게이트는 `local`(= LocalApplianceClient) — 세탁기는 이 폴러를 아예 안 탄다.
        const off = buildSyntheticOff({
          err: e, streak: failStreak + 1, wasRunning: running, unitCount: 1, enabled: local,
        });
        if (!off) throw e;
        comps = off;
        running = false;
        // ⚠️같은 사건을 홈킷 경로가 이미 info 로 알린다 — 여기서 또 찍으면 두 줄이 된다.
        if (!synthOffAnnounced) {
          synthOffAnnounced = true;
          log.debug?.(`[MQTT] ${slug} 로컬 응답 없음 — 꺼짐으로 중계합니다`);
        }
      }
    ...
    if (logic && typeof logic === 'object' && lastSeenUsable(configDevice)) {
      logic._onPollOk = () => {
...
function attachWaterPurifier({ bridge, log, client, deviceId, configDevice, slug, label, platform }) {
  ...
  let outageStart = 0;   // 순단이 시작된 시각(최장 길이 계산용)
  ...
  startPoller({
    ...
    run: async () => {
      ...
      if (typeof client._stat === 'function') {
        ...
        if (alive) {
          ...
          //   (적대 리뷰). 끊겼다가 돌아온 순간에 한 번 센다.
            if (deadStreak > 0) {
            st.outages = (st.outages || 0) + 1;
            // ⚠️길이도 함께 기록한다 — 안 그러면 요약선이 `순단 2건 (최장 0초)` 로 찍혀
            //   **절반이 거짓**이 된다(적대 리뷰 F8).
            if (outageStart) {
              const dur = Date.now() - outageStart;
              if (dur > (st.longestMs || 0)) st.longestMs = dur;
            }
          }
          outageStart = 0;
          st.ok += 1;
      ...
      if (!alive) {
        if (deadStreak === 0) outageStart = Date.now();
        deadStreak += 1;
        ...
        // ★v2.14.12 — 최후 안전망을 정수기에도 연결한다.
        //   v2.14.11 은 이걸 `_withFallback` 의 streak 에만 걸었는데, 정수기 getter 는 그 경로를
        //   하나도 타지 않는다(위 주석 참조) → **8/13 4시간 39분 사고를 겪은 당사자가 안전망 밖**이었다.
        //   임계·쿨다운·상한 판정은 전부 maybeRestartBridge 안에 있다.
        if (client && typeof client.maybeRestartBridge === 'function') {
          client.maybeRestartBridge(deviceId, deadStreak, label);
        }
