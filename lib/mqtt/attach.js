      ...
      startPoller({
        ...
        run: async () => {
          ...
          bridge.publishSmartAcSensors(slug, {
            ...
            mode_actual: (typeof mode === 'string' && mode) ? mode : undefined,
            // ★아래 둘은 홈킷에 자리가 없어 그동안 **아무 데도 안 가던** 값이다.
            wind_strength: wind ? wind.label : undefined,
            convenient_mode: conv || undefined,
          });
        },
      });
      ...
      startPoller({
        ...
        run: async () => {
          ...
          bridge.publishSmartAcSensors(slug, {
            ...
            autoclean_progress: clean ? (clean.progress != null ? clean.progress : null) : undefined,
            autoclean_running: clean ? clean.running : undefined,
            alarm_code: alarm ? (alarm.code || null) : undefined,
            alarm_ok: alarm ? alarm.ok : undefined,
            selfcheck_status: self ? self.status : undefined,
            night_mode: sleep ? sleep.night_mode : undefined,          // 열대야쾌면
            night_elapsed_min: sleep ? sleep.elapsed_min : undefined,
          });
        },
      });
    }
  ...
  if (local && deviceId) {
    ...
    const pollLocal = async () => {
      ...
      bridge.publishLaundryState(slug, {
        ...
        cumulative_kwh: energy ? energy.cumulative_kwh : null,
        power_w: energy ? energy.power_w : null,   // 같은 응답에 실려 오던 값(추가 조회 0회)
      });
    };
    ...
    startPoller({
      ...
      run: async () => {
        ...
        bridge.publishLaundryState(slug, {
          remote_control: (remote === true || remote === false) ? remote : undefined,
          kids_lock: kids || undefined,
          dry_level: setting ? setting.dryLevel : undefined,
          wrinkle_prevent: setting ? setting.wrinklePrevent : undefined,
          alarm_code: alarm ? (alarm.code || null) : undefined,   // 해제 시 null 을 살린다(M-1)
          alarm_ok: alarm ? alarm.ok : undefined,
          course: course ? course.code : undefined,               // 어떤 코스로 도는가
        });
      },
    });
  } else {
...
function attachWaterPurifier({ bridge, log, client, deviceId, configDevice, slug, label, platform }) {
  ...
  startPoller({
    ...
    run: async () => {
      ...
      // 다섯이 전부 null = 기기가 응답하지 않는다(부분 실패는 정상 — 리소스가 없는 것일 수 있다).
      ...
      if (typeof client._stat === 'function') {
        ...
        if (alive) st.ok += 1; else st.fail += 1;
      }
      if (!alive) {
        deadStreak += 1;
      ...
      bridge.publishWaterPurifierState(slug, {
        filter_remain: filter ? filter.remain_percent : undefined,
        filter_status: filter ? filter.status : undefined,
        status: status ? status.status : undefined,
        filter_door: status ? status.filter_door : undefined,
        sterilize_last: status ? status.sterilize_last : undefined,
        sterilize_next: status ? status.sterilize_next : undefined,
        sterilize_running: status ? status.sterilize_running : undefined,
        hot_temp: setting ? setting.hot_temp : undefined,
        capacity_ml: setting ? setting.capacity_ml : undefined,
        pouring: setting ? setting.pouring : undefined,
        lock_hot: locks ? locks.hot : undefined,
        lock_cold: locks ? locks.cold : undefined,
        lock_buzz: locks ? locks.buzz : undefined,
        selfcheck_status: self ? self.status : undefined,
        favorite_enabled: fav ? fav.enabled : undefined,      // 앱의 '나만의 출수량' 토글
        favorite_ml: fav ? fav.default_ml : undefined,
      });
    },
  });
  return true;
