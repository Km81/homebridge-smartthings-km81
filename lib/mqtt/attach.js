      ...
      startPoller({
        ...
        run: async () => {
          ...
          bridge.publishSmartAcSensors(slug, {
            ...
            autoclean_progress: clean && clean.progress != null ? clean.progress : undefined,
            autoclean_running: clean ? clean.running : undefined,
            alarm_code: alarm && alarm.code ? alarm.code : undefined,
...
function attachLaundry({ bridge, api, log, accessory, logic, configDevice, slug, kind, platform }) {
  ...
  if (local && deviceId) {
    ...
    const busySec = Math.max(10, Math.min(idleSec, 10));            // 가동 중(하한 10초)
    startPoller({
      name: `${slug} fast`, log, platform, firstDelaySec: 4,
      // ★함수를 주면 매 회차 다시 묻는다 — 상태가 바뀌면 다음 주기부터 따라간다.
      interval: () => (running ? busySec : idleSec),
      run: pollLocal,
    });
...
function attachWaterPurifier({ bridge, log, client, deviceId, configDevice, slug, label, platform }) {
  if (!bridge || !client || !deviceId) return false;
