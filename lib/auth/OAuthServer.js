...
// v2.6.8 — 인증 URL을 안내한 뒤 이 시간이 지나도 콜백이 없으면 원인을 한 번 짚어 준다.
const CALLBACK_HINT_MS = 10 * 60 * 1000;
...
class OAuthServer {
  ...
  start(onAuthenticated) {
    ...
    const launch = () => {
      ...
      this.server.listen(this.port, () => {
        ...
        // ★권한을 허용해도 콜백이 안 돌아오는 경우가 첫 설치의 흔한 벽이다(v2.6.8).
        //   원인은 거의 리버스 프록시다 — SmartThings는 https만 받고 우리 서버는 평문이라,
        //   중간에서 넘겨주지 않으면 브라우저만 성공하고 홈브릿지는 영영 기다린다.
        //   그동안 로그에는 아무 단서도 없었다. 조용히 기다리다 한 번만 짚어 준다.
        clearTimeout(this._silentTimer);
        this._silentTimer = setTimeout(() => {
          this.log.warn(`인증을 시작한 지 ${CALLBACK_HINT_MS / 60000}분이 지났는데 콜백이 오지 않았습니다.`);
          this.log.warn(`※ 브라우저에서 권한을 허용하셨다면, ${this.config.redirectUri} 가`
            + ` 리버스 프록시를 거쳐 이 홈브릿지의 ${this.port} 포트에 닿는지 확인하세요`
            + ' (SmartThings는 https만 받고 이 서버는 평문 http입니다).');
        }, CALLBACK_HINT_MS);
        if (this._silentTimer.unref) this._silentTimer.unref();
      });
    };
  ...
  stop() {
    // 인증이 끝났거나(성공 시 stop을 부른다) 종료 중이면 '콜백이 안 온다' 안내는 필요 없다.
    clearTimeout(this._silentTimer);
  ...
  async _handleOAuthCallback(req, res, reqUrl, onAuthenticated) {
    ...
    this.log.info('인증 코드 수신됨 — 토큰 발급 중...');
