...
- 토큰은 필요 없습니다.
- **준비물 2가지** — 이게 없으면 로컬이 안 붙습니다(가장 흔한 첫 설치 실패):
  1. **파이썬 3 + pip** — 신형 기기는 DTLS로 통신하는데 Node에 DTLS가 없어 파이썬 도우미를 씁니다.
     플러그인이 **첫 기동 때 자동으로 설치**합니다(`smartthings-local`).
  2. **처음 한 번의 인터넷** — 파이썬 패키지 다운로드 + 기기와 통신할 인증서 발급에 필요합니다.
     한 번 끝내면 그 뒤로는 인터넷 없이 동작합니다.
...
> | `파이썬을 찾을 수 없습니다` | 파이썬 3을 설치하거나, 설정 `localPythonBin`에 실행 경로를 지정 |
...
>
> 설치가 실패해도 **다음 재시작에서 자동으로 다시 시도**합니다.
