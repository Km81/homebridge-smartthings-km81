...
| **`SmartThings deviceId`** | `a1b2c3d4-…` 형태의 고유 번호 | 이름으로 한 번 붙인 뒤 (권장) | 불필요 |
...
#### 먼저 이 조합으로 해 보세요 (실사용 검증된 설정)

개발 환경의 2in1(거실+침실)에서 **실제로 동작하는** 값입니다. 쓰기가 읽기와 **교차**입니다.
...
(참고: 이 저장소 개발 환경의 2in1은 **쓰기만 교차**였습니다.)

### 신형 에어컨·건조기 (2023년 이후)

⚠️**구형과 달리 순서가 있습니다.** 구형은 IP와 토큰만 있으면 바로 끝나지만, 신형은
**SmartThings 계정 연결이 먼저**입니다. 기기 토큰은 필요 없는 대신, 어느 기기인지 지목할
`deviceId`를 얻어야 하고 그 수단이 SmartThings이기 때문입니다.
...
3단계  로그에서 deviceId를 복사해 설정에 붙여넣기   ← 이제 부팅 때 클라우드 조회 0회
   ↓
4단계  (원하면) '로컬 실패 시 클라우드 사용' 끄기   ← 완전 로컬. 클라우드 호출 0회
```

2단계까지만 해도 동작합니다. 3·4단계는 **클라우드 의존을 없애는** 선택 과정입니다
(2026년 10월 SmartThings API 유료화 대비).

#### 1단계 — SmartThings 연결

[SmartThings 클라우드를 쓸 때](#smartthings-클라우드를-쓸-때) 절을 먼저 끝내세요.
⚠️여기에 **https 리버스 프록시**가 필요합니다 — 이게 첫 설치에서 가장 높은 벽입니다.

#### 2단계 — 기기 설정

`장치 종류`를 **신형 에어컨** 또는 **건조기**로 고르고 아래를 채웁니다.
...
**준비물 3가지** — 이게 없으면 로컬이 안 붙습니다(가장 흔한 첫 설치 실패):
...
**deviceId를 어디서 찾아 헤맬 필요가 없습니다** — 플러그인이 알려줍니다. 이 값을
`SmartThings deviceId` 칸에 붙여넣고 재시작하면, 그 뒤로는 부팅할 때 **클라우드를 아예 안 부릅니다.**

#### 4단계 — 완전 로컬로 (선택)

`로컬 실패 시 클라우드 사용`을 끄면 이 기기는 SmartThings API를 **한 번도** 쓰지 않습니다.
⚠️대신 로컬이 실패하면 **그 기기는 제어되지 않습니다**(홈 앱에 `응답 없음`).
기기 IP가 바뀌거나 파이썬 도우미가 깨지면 기댈 곳이 없다는 뜻이니, 안정된 것을 확인한 뒤 끄세요.
...
파이썬 버전을 바꾸시면(예: `localPythonBin` 변경) **알아서 다시 설치**하니 신경 쓰지 않으셔도 됩니다.
...
> | `설치할 수 있는 버전을 찾지 못했습니다` | 위와 같습니다(네트워크 문제가 아닙니다) |
...
> **꺼져 있으면 그 기기는 제어되지 않습니다**(홈 앱에 `응답 없음`) — 급하면 그 옵션을 켜세요.
