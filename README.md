...
세탁/건조가 끝나는 순간 **iOS 푸시 알림 1회**를 받기 위한 가상 **모션 센서**를 별도 액세서리로 노출합니다. HomeKit 자동화 트리거로도 사용 가능합니다.
...
- 직전 상태가 `active`였다가 `inactive`로 바뀌는 순간 모션 센서를 `detected`로 트리거 → 약 10초 후 자동으로 해제
- 모션 센서를 선택한 이유: iOS HomeKit의 푸시 알림이 `motion=false → true` 전환에서만 발생하므로 **운전 종료 시 정확히 1회**만 알림이 옴 (Contact/Occupancy는 양방향이라 2회 발송됨)
- 10초 후 자동 해제는 다음 종료 사이클에서 또 알림을 받기 위해 필수
- 폴링은 `enableNotificationSensor: true`인 장치에만 시작됨
...
| 시나리오 | 트리거 | 동작 |
| --- | --- | --- |
| 세탁 완료 시 거실 조명 깜빡임 | "세탁기 종료알림"에서 동작 감지되면 | 거실 조명 On → 1분 후 Off |
| 건조 완료 시 가족에게 알림 | "건조기 종료알림"에서 동작 감지되면 | "건조 끝났어요" 메시지 전송 |
...
### 전원 ON 직후 다중 명령 보호 (v1.7.0+)

구형 삼성 AC 펌웨어는 짧은 시간 안에 여러 명령(전원/모드/온도/스윙)을 받으면 일부를 누락하거나 순서를 뒤섞는 문제가 있습니다. 이 플러그인은 **전원을 켠 직후 잠시 다른 명령을 가로채는 보호 윈도우**를 둡니다.

| 설정 | 기본값 | 의미 |
|---|---|---|
| `legacyOnGuardMs` | `2000` (ms) | ON 명령 직후 보호 시간. `0`이면 보호 꺼짐. |
| `legacyOnGuardStrategy` | `"drop"` | `drop` = 보호 시간 안에 들어온 다른 명령을 무시. `queue` = 보호 시간 종료 후 마지막 값으로 한 번에 발사. |

**언제 어떤 전략을 쓸지**
- iOS 홈 앱에서 손으로 전원만 켜고 이후 별도로 온도/스윙을 조작 → **`drop`(기본)이 가장 안전**합니다. 구형 펌웨어가 받는 명령을 ON 한 개로 보장합니다.
- HomeKit 자동화/Siri로 "켜고 26도로 설정" 같은 일괄 동작을 쓴다면 → **`queue`로 변경**하세요. 보호 시간이 끝난 뒤 누락 없이 순차 전송됩니다.

**로그로 확인할 수 있는 동작**
```
[거실 에어컨] ON 보호 시작 — 이후 2000ms 동안 다른 명령은 무시됩니다.
[거실 에어컨] ON 보호 중 명령 무시: /temperatures/0 -> {"desired":26}
[거실 에어컨] ON 보호 종료 (expired)
```
`drop` 전략에서 의도하지 않게 명령이 무시되는 경우, 위 로그가 단서가 됩니다.

> 같이 도입된 보호 기능: 같은 값으로 set이 와도 명령을 보내지 않는 *idempotency no-op*, Swing/Lock의 `/mode` 옵션 상호 덮어쓰기를 막는 *옵션 병합*, 빠른 연속 set 시 1회로 묶는 *single refresh*, 끄는 순간 대기 중인 명령을 모두 취소하는 *pending cancel* 등 — 전부 자동 동작합니다.
