...
어느 기기인지 지목할 `deviceId`가 필요합니다. **`deviceId`를 이미 알고 있으면 아래 흐름을
건너뛰고 바로 넣으면 됩니다** — 그 경우 SmartThings 연결은 필요 없습니다(아래 '완전 로컬' 참고).
모르는 경우, 이름으로 찾아 알아내는 수단이 SmartThings 클라우드입니다.
...
대신 로컬이 실패하면 **그 기기는 제어되지 않습니다**(홈 앱에 `응답 없음`).

아래 세 조건을 **모두** 만족하는 기기만 있으면 `clientId`·`clientSecret`·`redirectUri`를
**비워 둬도 됩니다.** OAuth 연결도, 리버스 프록시도 필요 없습니다.

- `SmartThings deviceId`를 적었다
- `전송 경로`가 **로컬**이다
- `로컬 실패 시 클라우드 사용`이 꺼져 있다

이 구성이면 부팅 로그에 `SmartThings 연결 없이 로컬 전용으로 동작합니다 (클라우드 호출 0회).`가
나옵니다. 하나라도 어긋나면 OAuth 항목이 필요하다는 오류가 뜹니다.

> 즉 1단계(OAuth)는 **`deviceId`를 알아내기 위한 수단**입니다. 다른 방법으로 이미 알고 있다면
> 처음부터 건너뛸 수 있습니다.
