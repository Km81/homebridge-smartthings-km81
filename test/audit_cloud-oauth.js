...
// ═══════════════════════════════════════════════════════════════════
{
  ...
  // F3. 실패 경로 — 라벨+명령+상태코드, 에러 body는 debug로 강등(v2.1.1)
  const log2 = mkLog();
  ...
  assert(count(log2.lines.error, 'ConstraintViolationError') === 0, 'F3 ★ 에러 body가 error 레벨에 없음');
  assert(count(log2.lines.debug, 'ConstraintViolationError') === 1, 'F3 ★ 에러 body는 debug로만 — 기본 로그레벨에선 원인 코드 소실');
