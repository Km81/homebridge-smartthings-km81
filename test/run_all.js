...
const SUITES = [
  ...
  // v2.15.0 — 「임시 연결해제(putAway)」. 이 기능의 목적은 **로그를 안 내는 것**인데,
  //           로그 문구는 NAS 감시기의 API 라서 어휘가 한 조각만 섞여도 기능이 스스로
  //           감시를 깨운다. ★대조군 포함 — "0건"이 「안 돌아서 0」이 아님을 같이 잰다.
  { file: 'put_away.js' },
  // v2.16.1 — 2.16.0 의 put_away 는 초록인데 SmartAC·세탁기 봉인이 무효였다(모형이 옛 리스너를 몰랐다).
  //           플랫폼 계층 누수(기동 신원 조회·브릿지 등록)와, **실제 hap-nodejs** 로 읽고 누르는 시험을 더한다.
  //           ⛔hap_js 는 hap 이 없으면 skip 이 아니라 실패한다(devDependencies 로 설치된다).
  { file: 'put_away_platform.js' },
  { file: 'put_away_hap.js' },
  ...
  { file: 'wind_seams.js' },
  // ★★2026-08-03 — 아래 3종은 v2.11.0~2.11.3에서 만들고 **여기 등록을 빠뜨렸다.**
  //   그동안 `npm test`(= publish 게이트)가 24스위트만 돌아, 그날 봉합한 결함들을
  //   **릴리스 게이트가 하나도 지키지 않았다.** 커밋 메시지의 "27/28/29스위트"는
  //   수동 실행 기준이었다. 적대 리뷰가 잡았다(M4).
